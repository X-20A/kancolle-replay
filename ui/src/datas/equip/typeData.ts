import { DeepReadonly } from "@/types";
import { AACITriggerEquipType, SkillTriggerEquipType } from "@/types/equip/player";
import { EquipType } from "./base/player";
import { ShipType } from "@/wasm/kssw";

/**
 * 装備種別ごとの諸元
 */
export type EquipTypeData = {
    label: string,
    icon: number,
    b_type?: SkillTriggerEquipType,
    a_type?: AACITriggerEquipType,
    can_equip_ship_types: ShipType[],
    can_equip_ship_ids?: number[],
    can_not_equip_ship_ids?: number[],
    can_asw_damage?: true,
    can_equip_land_base?: true,
    is_plane?: true,
    is_fighter?: true,
    is_torpedo_bomber?: true,
    can_contact?: true,
    can_detect?: true,
    is_dive_bomber?: true,
    is_asw_plane?: true,
    is_land_base_plane?: true,
    is_jet?: true,
    can_support_asw?: true,
}

export type EquipTypeDatas = DeepReadonly<Record<EquipType, EquipTypeData>>;

export const EQUIP_TYPE_DATAS: EquipTypeDatas = {
    [EquipType.MAIN_GUN_S]: {
        label: 'Main Gun (S)',
        icon: 1,
        b_type: SkillTriggerEquipType.B_MAINGUN,
        a_type: AACITriggerEquipType.A_GUN,
        can_equip_ship_types: ["DD", "CL", "CT", "CLT", "AS", "AV", "AO", "AR", "LHA", "DE"],
        can_equip_ship_ids: [541],
        can_not_equip_ship_ids: [507, 645, 650, 717, 900, 943],
    },
    [EquipType.MAIN_GUN_S_AA]: {
        label: 'Main Gun (S)',
        icon: 16,
        b_type: SkillTriggerEquipType.B_MAINGUN,
        a_type: AACITriggerEquipType.A_HAGUN,
        can_equip_ship_types: ["DD", "CL", "CT", "CLT", "AS", "AV", "AO", "AR", "LHA", "DE"],
        can_equip_ship_ids: [541],
        can_not_equip_ship_ids: [507, 645, 650, 717, 900, 943],
    },
    [EquipType.MAIN_GUN_M]: {
        label: 'Main Gun (M)',
        icon: 2,
        b_type: SkillTriggerEquipType.B_MAINGUN,
        a_type: AACITriggerEquipType.A_GUN,
        can_equip_ship_types: ["CL", "CT", "CLT", "CA", "CAV", "BB", "BBV"],
        can_equip_ship_ids: [372, 491, 500, 507, 581, 586, 634, 635, 639, 640, 690, 911],
        can_not_equip_ship_ids: [877],
    },
    [EquipType.MAIN_GUN_L]: {
        label: 'Main Gun (L)',
        icon: 3,
        b_type: SkillTriggerEquipType.B_MAINGUN,
        a_type: AACITriggerEquipType.A_GUN,
        can_equip_ship_types: ["FBB", "BB", "BBV"],
    },
    [EquipType.MAIN_GUN_XL]: {
        label: 'Main Gun (L)',
        icon: 3,
        b_type: SkillTriggerEquipType.B_MAINGUN,
        a_type: AACITriggerEquipType.A_GUN,
        can_equip_ship_types: [],
        can_equip_ship_ids: [131, 136, 143, 148, 275, 276, 541, 546, 573, 911, 916],
    },
    [EquipType.SECONDARY_GUN]: { // (分類A)
        label: 'Secondary Gun',
        icon: 4,
        b_type: SkillTriggerEquipType.B_SECGUN,
        a_type: AACITriggerEquipType.A_GUN,
        can_equip_ship_types: ["CL", "CT", "CLT", "CA", "CAV", "FBB", "BB", "BBV", "CVL", "CV", "CVB", "AS", "AV", "AR", "LHA"],
        can_equip_ship_ids: [352, 460, 500, 699, 717],
        can_not_equip_ship_ids: [477, 478, 622, 623, 624, 945],
    },
    [EquipType.SECONDARY_GUN_AA]: { // (分類B)
        label: 'Secondary Gun',
        icon: 16,
        b_type: SkillTriggerEquipType.B_SECGUN,
        a_type: AACITriggerEquipType.A_HAGUN,
        can_equip_ship_types: ["CL", "CT", "CLT", "CA", "CAV", "FBB", "BB", "BBV", "CVL", "CV", "CVB", "AS", "AV", "AR", "LHA"],
        can_equip_ship_ids: [352, 460, 500, 699, 717],
        can_not_equip_ship_ids: [477, 478, 622, 623, 624, 945],
    },
    [EquipType.SECONDARY_GUN_L]: { // (分類C)
        label: 'Secondary Gun',
        icon: 4,
        b_type: SkillTriggerEquipType.B_SECGUN,
        a_type: AACITriggerEquipType.A_GUN,
        can_equip_ship_types: ["CA", "FBB", "BB", "BBV", "CV", "CVB"],
    },
    [EquipType.AP_SHELL]: {
        label: 'AP Shell',
        icon: 13,
        b_type: SkillTriggerEquipType.B_APSHELL,
        can_equip_ship_types: ["FBB", "BB", "BBV"],
        can_not_equip_ship_ids: [877],
    },
    [EquipType.TORPEDO]: {
        label: 'Torpedo',
        icon: 5,
        b_type: SkillTriggerEquipType.B_TORPEDO,
        can_equip_ship_types: ["DD", "CL", "CT", "CLT", "CA", "CAV", "SS", "SSV"],
        can_equip_ship_ids: [178, 507, 591, 592, 593, 877, 879, 954],
        can_not_equip_ship_ids: [657],
    },
    [EquipType.TORPEDO_SS]: {
        label: 'Torpedo',
        icon: 5,
        b_type: SkillTriggerEquipType.B_TORPEDO,
        can_equip_ship_types: ["SS", "SSV"],
    },
    [EquipType.MIDGET_SUBMARINE]: {
        label: 'Midget Sub',
        icon: 5,
        can_equip_ship_types: ["CLT", "SS", "SSV", "AV"],
        can_equip_ship_ids: [200, 488, 506, 623, 657, 668],
        can_not_equip_ship_ids: [372, 445, 450, 491, 539, 605, 731, 939],
    },
    [EquipType.AA_GUN]: {
        label: 'Anti-Air Gun',
        icon: 15,
        a_type: AACITriggerEquipType.A_AAGUN,
        can_equip_ship_types: ["DD", "CL", "CT", "CLT", "CA", "CAV", "FBB", "BB", "BBV", "CVL", "CV", "CVB", "AS", "AV", "AO", "AR", "LHA", "DE"],
        can_equip_ship_ids: [530, 539, 939, 940],
        can_not_equip_ship_ids: [645, 650],
    },
    [EquipType.ANTI_AIR_FIRE_DIRECTOR]: {
        label: 'Fire Director',
        icon: 30,
        a_type: AACITriggerEquipType.A_AAFD,
        can_equip_ship_types: ["DD", "CL", "CT", "CLT", "CA", "CAV", "FBB", "BB", "BBV", "CVL", "CV", "CVB", "AS", "AV", "AO", "AR", "LHA", "DE"],
        can_not_equip_ship_ids: [621, 645, 650, 727, 877, 945],
    },
    [EquipType.SONAR_S]: {
        label: 'Sonar',
        icon: 18,
        b_type: SkillTriggerEquipType.B_SONAR,
        can_equip_ship_types: ["DD", "CL", "CT", "CLT", "SS", "SSV", "DE"],
        can_equip_ship_ids: [352, 372, 380, 381, 382, 450, 491, 500, 507, 529, 534, 536, 546, 554, 586, 591, 592, 593, 626, 645, 650, 690, 699, 707, 713, 717, 879, 885, 889, 894, 899, 900, 911, 916, 943, 948, 954],
        can_asw_damage: true,
    },
    [EquipType.SONAR_L]: {
        label: 'Sonar',
        icon: 18,
        b_type: SkillTriggerEquipType.B_SONAR,
        can_equip_ship_types: ["CA", "CAV", "FBB", "BB", "BBV", "CVL", "CV", "CVB", "AS", "AV", "LHA"],
        can_equip_ship_ids: [330, 346, 357, 500, 537, 538, 624, 629, 650, 662, 663, 668, 717],
        can_not_equip_ship_ids: [380, 381, 382, 521, 522, 526, 534, 581, 727, 877, 884, 943, 945],
        can_asw_damage: true,
    },
    [EquipType.DEPTH_CHARGE]: {
        label: 'Depth Charge',
        icon: 17,
        b_type: SkillTriggerEquipType.B_DEPTHCHARGE,
        can_equip_ship_types: ["DD", "CL", "CT", "CLT", "AV", "DE"],
        can_equip_ship_ids: [380, 381, 382, 411, 412, 500, 529, 534, 536, 699, 717, 889, 900],
        can_not_equip_ship_ids: [372, 491, 507, 581, 586, 690],
        can_asw_damage: true,
    },
    [EquipType.FIGHTER]: {
        label: 'Fighter',
        icon: 6,
        can_equip_ship_types: ["CVL", "CV", "CVB", "LHA"],
        can_equip_ship_ids: [553, 554, 717],
        can_not_equip_ship_ids: [621, 626, 727, 943, 945],
        can_equip_land_base: true,
        is_plane: true,
        is_fighter: true,
    },
    [EquipType.TORPEDO_BOMBER]: {
        label: 'Torpedo Bomber',
        icon: 8,
        can_equip_ship_types: ["CVL", "CV", "CVB"],
        can_equip_ship_ids: [352],
        can_not_equip_ship_ids: [521, 899],
        can_equip_land_base: true,
        is_plane: true,
        is_fighter: true,
        is_torpedo_bomber: true,
        can_contact: true,
        is_asw_plane: true,
        can_asw_damage: true,
        can_support_asw: true,
    },
    [EquipType.DIVE_BOMBER]: {
        label: 'Dive Bomber',
        icon: 7,
        can_equip_ship_types: ["CVL", "CV", "CVB"],
        can_equip_ship_ids: [553, 554, 717, 948],
        can_equip_land_base: true,
        is_plane: true,
        is_fighter: true,
        is_dive_bomber: true,
        is_asw_plane: true,
        can_asw_damage: true,
        can_support_asw: true,
    },
    [EquipType.FIGHTER_BOMBER]: {
        label: 'Dive Bomber',
        icon: 7,
        can_equip_ship_types: ["CVL", "CV", "CVB"],
        can_equip_ship_ids: [553, 554, 717, 948],
        can_equip_land_base: true,
        is_plane: true,
        is_fighter: true,
        is_dive_bomber: true,
        is_asw_plane: true,
        can_asw_damage: true,
        can_support_asw: true,
    },
    [EquipType.SEAPLANE]: {
        label: 'Recon Seaplane',
        icon: 10,
        b_type: SkillTriggerEquipType.B_RECON,
        can_equip_ship_types: ["CL", "CT", "CA", "CAV", "FBB", "BB", "BBV", "SSV", "AS", "AV", "AO"],
        can_equip_ship_ids: [621, 626],
        can_not_equip_ship_ids: [477, 478, 622, 623, 624, 645, 650, 657, 699, 717, 900],
        can_equip_land_base: true,
        is_plane: true,
        can_contact: true,
        can_detect: true,
        can_support_asw: true,
    },
    [EquipType.SEAPLANE_BOMBER]: {
        label: 'Seaplane Bomber',
        icon: 10,
        b_type: SkillTriggerEquipType.B_RECON,
        can_equip_ship_types: ["CAV", "BBV", "SSV", "AV", "AO"],
        can_equip_ship_ids: [305, 306, 307, 314, 358, 361, 392, 446, 447, 488, 496, 547, 574, 579, 591, 593, 626, 630, 639, 640, 652, 662, 663, 668, 724, 879, 911, 954],
        can_not_equip_ship_ids: [645, 650, 699, 717, 900],
        can_equip_land_base: true,
        is_plane: true,
        is_fighter: true,
        is_dive_bomber: true,
        can_detect: true,
        is_asw_plane: true,
        can_asw_damage: true,
        can_support_asw: true,
    },
    [EquipType.CARRIER_SCOUT]: {
        label: 'Scout Plane',
        icon: 9,
        can_equip_ship_types: ["CVL", "CV", "CVB"],
        can_equip_ship_ids: [553, 554],
        can_not_equip_ship_ids: [380, 381, 382, 521, 522, 526, 534, 884],
        can_equip_land_base: true,
        is_plane: true,
        can_contact: true,
        can_detect: true,
    },
    [EquipType.CARRIER_SCOUT_2]: {
        label: 'Scout Plane',
        icon: 9,
        can_equip_ship_types: ["CVB"],
        can_equip_land_base: true,
        is_plane: true,
        can_contact: true,
        can_detect: true,
    },
    [EquipType.AUTOGYRO]: {
        label: 'Anti-Sub Plane',
        icon: 21,
        can_equip_ship_types: ["CAV", "BBV", "CVL", "AS", "AO", "AR", "LHA"],
        can_equip_ship_ids: [372, 477, 478, 491, 546, 547, 573, 574, 586, 630, 646, 652, 662, 663, 668, 690, 713, 885, 911, 920],
        can_not_equip_ship_ids: [380, 381, 382, 521, 522, 526, 534, 621, 645, 699, 727, 884, 889, 900, 943, 945],
        can_equip_land_base: true,
        is_plane: true,
        is_fighter: true,
        is_dive_bomber: true,
        is_asw_plane: true,
        can_asw_damage: true,
        can_support_asw: true,
    },
    [EquipType.ASW_PLANE]: {
        label: 'Anti-Sub Plane',
        icon: 22,
        can_equip_ship_types: ["CVL", "LHA"],
        can_equip_ship_ids: [553, 554, 646, 717, 900],
        can_not_equip_ship_ids: [521, 522, 526, 534, 621, 626, 727, 884, 894, 945],
        can_equip_land_base: true,
        is_plane: true,
        is_fighter: true,
        is_dive_bomber: true,
        is_asw_plane: true,
        can_asw_damage: true,
        can_support_asw: true,
    },
    [EquipType.RADAR_S]: {
        label: 'Radar (S)',
        icon: 11,
        b_type: SkillTriggerEquipType.B_RADAR,
        can_equip_ship_types: ["DD", "CL", "CT", "CLT", "CA", "CAV", "FBB", "BB", "BBV", "CVL", "CV", "CVB", "AS", "AV", "AO", "AR", "LHA", "DE"],
        can_not_equip_ship_ids: [945],
    },
    [EquipType.RADAR_L]: {
        label: 'Radar (L)',
        icon: 11,
        b_type: SkillTriggerEquipType.B_RADAR,
        can_equip_ship_types: ["CL", "CT", "CLT", "CA", "CAV", "FBB", "BB", "BBV", "CVL", "CV", "CVB", "AV"],
        can_equip_ship_ids: [179, 180, 330, 346, 352, 357, 419, 421, 422, 423, 470, 532, 533, 537, 538, 626, 645, 650, 699, 717, 948],
        can_not_equip_ship_ids: [477, 478, 521, 522, 574, 623, 624, 877],
    },
    [EquipType.RADAR_XL]: {
        label: 'Radar (L)',
        icon: 11,
        b_type: SkillTriggerEquipType.B_RADAR,
        can_equip_ship_types: ["FBB", "BB", "BBV"],
        can_not_equip_ship_ids: [877],
    },
    [EquipType.ENGINE]: {
        label: 'Engine',
        icon: 19,
        can_equip_ship_types: ["DD", "CL", "CT", "CLT", "CA", "CAV", "FBB", "BB", "BBV", "CVL", "CV", "CVB", "SS", "SSV", "AS", "AV", "AO", "AR", "LHA"],
        can_not_equip_ship_ids: [],
    },
    [EquipType.TYPE_3_SHELL]: {
        label: 'Anti-Air Shell',
        icon: 12,
        a_type: AACITriggerEquipType.A_TYPE3SHELL,
        can_equip_ship_types: ["CA", "CAV", "FBB", "BB", "BBV"],
        can_not_equip_ship_ids: [877],
        can_equip_ship_ids: [507],
    },
    [EquipType.BULGE_M]: {
        label: 'Torpedo Bulge',
        icon: 23,
        can_equip_ship_types: ["CT", "CA", "CAV", "CVL", "AS", "AV", "AR"],
        can_equip_ship_ids: [146, 147, 179, 180, 216, 217, 305, 306, 307, 314, 330, 346, 357, 500, 537, 538, 542, 543, 547, 556, 559, 563, 564, 566, 567, 568, 569, 578, 579, 621, 622, 623, 624, 626, 629, 630, 645, 649, 650, 652, 656, 657, 662, 663, 668, 670, 699, 717, 727, 900, 915, 943, 948, 951, 955, 960, 961],
    },
    [EquipType.BULGE_L]: {
        label: 'Torpedo Bulge',
        icon: 23,
        can_equip_ship_types: ["FBB", "BB", "BBV", "CV", "CVB"],
        can_equip_ship_ids: [650],
    },
    [EquipType.LANDING_CRAFT]: {
        label: 'Misc',
        icon: 20,
        b_type: SkillTriggerEquipType.B_LC1,
        can_equip_ship_types: ["AV", "LHA"],
        can_equip_ship_ids: [147, 198, 199, 200, 260, 352, 382, 418, 419, 434, 435, 464, 468, 469, 470, 478, 487, 488, 489, 490, 498, 500, 506, 541, 547, 548, 559, 563, 587, 623, 630, 645, 647, 650, 657, 666, 667, 699, 703, 707, 716, 718, 720, 725, 889, 899, 908, 915, 916, 951, 959, 960, 975],
        can_not_equip_ship_ids: [445, 491, 727, 945],
    },
    [EquipType.SEARCHLIGHT_S]: {
        label: 'Night Equip',
        icon: 24,
        can_equip_ship_types: ["DD", "CL", "CA", "CAV", "FBB", "BB", "BBV", "AV"],
        can_equip_ship_ids: [343, 356, 500, 626, 639, 640, 645, 650, 699, 727, 948],
    },
    [EquipType.SEARCHLIGHT_L]: {
        label: 'Night Equip',
        icon: 24,
        can_equip_ship_types: ["FBB", "BB", "BBV"],
        can_equip_ship_ids: [372, 477, 491, 496, 501, 502, 506, 507, 579, 586, 626, 630, 645],
    },
    [EquipType.STARSHELL]: {
        label: 'Night Equip',
        icon: 27,
        can_equip_ship_types: ["DD", "CL", "CLT", "CA", "CAV", "FBB", "BB", "BBV", "AS", "AV", "AR"],
        can_equip_ship_ids: [500, 699, 727],
    },
    [EquipType.PICKET]: {
        label: 'Night Equip',
        icon: 32,
        can_equip_ship_types: ["DD", "CL", "CT", "CLT", "CA", "CAV", "FBB", "BB", "BBV", "AS", "AV", "DE"],
        can_equip_ship_ids: [381, 500, 529, 536, 621, 626, 699, 889, 894, 899],
    },
    [EquipType.WG42]: {
        label: 'Misc',
        icon: 31,
        can_equip_ship_types: ["DD", "CL", "CAV", "BBV", "SS", "SSV", "AS", "AV", "LHA"],
        can_equip_ship_ids: [500, 573, 591, 592, 593, 699, 877, 878, 879, 954],
        can_not_equip_ship_ids: [445, 553, 554, 943, 948],
    },
    [EquipType.SRF]: {
        label: 'Misc',
        icon: 26,
        can_equip_ship_types: ["AR"],
        can_equip_ship_ids: [450],
    },
    [EquipType.FCF]: {
        label: 'Misc',
        icon: 28,
        can_equip_ship_types: ["CL", "CT", "CLT", "CA", "CAV", "FBB", "BB", "BBV", "CVL", "CV", "CVB", "AS", "AV", "LHA"],
        can_equip_ship_ids: [330, 346, 357, 419, 421, 422, 423, 464, 497, 498, 500, 532, 533, 537, 538, 542, 543, 567, 587, 628, 629, 645, 649, 650, 651, 656, 667, 699, 720, 915, 961, 975],
        can_not_equip_ship_ids: [521, 522, 727, 877, 943, 945, 948],
    },
    [EquipType.DRUM]: {
        label: 'Misc',
        icon: 25,
        can_equip_ship_types: ["DD", "CL", "CAV", "AV", "AO", "LHA"],
        can_equip_ship_ids: [530, 539, 605, 707, 731, 899, 939, 940],
        can_not_equip_ship_ids: [717, 900, 943, 948],
    },
    [EquipType.SCAMP]: {
        label: 'Misc',
        icon: 29,
        can_equip_ship_types: ["CAV", "BBV", "CVL", "CV", "CVB", "AO"],
        can_equip_ship_ids: [166, 372, 450, 488, 491, 496, 507, 547, 574, 579, 581, 586, 626, 630, 652, 662, 663, 668, 690, 879, 943, 948],
        can_not_equip_ship_ids: [645, 699],
    },
    [EquipType.FLYING_BOAT]: {
        label: 'Recon Seaplane',
        icon: 33,
        can_equip_ship_types: [],
        can_equip_ship_ids: [445, 450, 500, 586, 690],
        can_equip_land_base: true,
        is_plane: true,
        can_contact: true,
        can_detect: true,
        is_asw_plane: true,
        can_support_asw: true,
    },
    [EquipType.REPAIR]: {
        label: 'Misc',
        icon: 14,
        can_equip_ship_types: ["DD", "CL", "CT", "CLT", "CA", "CAV", "FBB", "BB", "BBV", "CVL", "CV", "CVB", "SS", "SSV", "AS", "AV", "AO", "AR", "LHA", "DE"],
    },
    [EquipType.RATION]: {
        label: 'Misc',
        icon: 34,
        can_equip_ship_types: ["DD", "CL", "CT", "CLT", "CA", "CAV", "FBB", "BB", "BBV", "CVL", "CV", "CVB", "SS", "SSV", "AS", "AV", "AO", "AR", "LHA", "DE"],
    },
    [EquipType.SEAPLANE_FIGHTER]: {
        label: 'Seaplane Fighter',
        icon: 43,
        can_equip_ship_types: ["CAV", "BBV", "SSV", "AS", "AV", "AO"],
        can_equip_ship_ids: [136, 148, 275, 276, 358, 361, 446, 447, 488, 496, 541, 546, 547, 573, 592, 593, 621, 626, 652, 668, 879, 911],
        can_not_equip_ship_ids: [645, 650, 699, 717, 900],
        can_equip_land_base: true,
        is_plane: true,
        is_fighter: true,
        can_support_asw: true,
    },
    [EquipType.LANDING_TANK]: {
        label: 'Misc',
        icon: 36,
        b_type: SkillTriggerEquipType.B_LC3,
        can_equip_ship_types: ["SS", "SSV", "AV", "LHA"],
        can_equip_ship_ids: [147, 198, 199, 200, 352, 418, 464, 468, 470, 478, 487, 488, 489, 490, 497, 500, 506, 541, 547, 548, 556, 564, 568, 569, 573, 578, 587, 588, 623, 647, 656, 657, 662, 663, 666, 667, 668, 670, 899, 908, 915, 916, 954, 960, 961],
        can_not_equip_ship_ids: [445, 491, 581, 943],
    },
    [EquipType.OILDRUM]: {
        label: 'Misc',
        icon: 35,
        can_equip_ship_types: ["AO"],
        can_equip_ship_ids: [943, 948],
    },
    [EquipType.LAND_BASED_BOMBER]: {
        label: 'Misc',
        icon: 37,
        can_equip_ship_types: [],
        can_equip_land_base: true,
        is_plane: true,
        is_dive_bomber: true,
        is_torpedo_bomber: true,
        is_land_base_plane: true,
    },
    [EquipType.INTERCEPTOR]: {
        label: 'Misc',
        icon: 38,
        can_equip_ship_types: [],
        can_equip_land_base: true,
        is_plane: true,
        is_land_base_plane: true,
    },
    [EquipType.LAND_BASED_SCOUT]: {
        label: 'Misc',
        icon: 9,
        can_equip_ship_types: [],
        can_equip_land_base: true,
        is_plane: true,
        can_contact: true,
    },
    [EquipType.TRANSPORT_ITEM]: {
        label: 'Transportation Material',
        icon: 41,
        can_equip_ship_types: ["CAV", "BBV", "CVL", "CV", "CVB", "SSV", "AV", "AR", "LHA"],
        can_equip_ship_ids: [500, 530, 539, 605, 645, 650, 699, 717, 731, 900, 939, 940],
        can_not_equip_ship_ids: [502, 507, 621, 626, 916],
    },
    [EquipType.SUBMARINE_RADAR]: {
        label: 'Submarine Equipment',
        icon: 42,
        can_equip_ship_types: ["SS", "SSV"],
    },
    [EquipType.LAND_BASED_BOMBER_L]: {
        label: 'Misc',
        icon: 49,
        can_equip_ship_types: [],
        can_equip_land_base: true,
        is_plane: true,
        is_dive_bomber: true,
        is_torpedo_bomber: true,
        is_land_base_plane: true,
    },
    [EquipType.JET_BOMBER]: {
        label: 'Jet Fighter-Bomber',
        icon: 39,
        can_equip_ship_types: [],
        can_equip_ship_ids: [466, 467, 646, 713],
        can_equip_land_base: true,
        is_plane: true,
        is_fighter: true,
        is_dive_bomber: true,
        is_jet: true,
    },
    [EquipType.ARMY_UNIT]: {
        label: 'Army Unit',
        icon: 52,
        can_equip_ship_types: [],
        can_equip_ship_ids: [727, 945],
    },
    [EquipType.SMOKESCREEN]: {
        label: 'Smoke Generator',
        icon: 54,
        can_equip_ship_types: ["DE", "DD", "CL", "CLT", "CA", "CT"],
        can_equip_ship_ids: [411, 412, 507, 645, 650, 699],
    },
    [EquipType.BARRAGE_BALLOON]: {
        label: 'Barrage Balloon',
        icon: 54,
        can_equip_ship_types: ["DE", "DD", "CL", "CLT", "CA", "CT"],
        can_equip_ship_ids: [411, 412, 507, 645, 650, 699],
    },
    [EquipType.OTHER]: {
        label: 'Misc',
        icon: 14,
        can_equip_ship_types: ["DD", "CL", "CT", "CLT", "CA", "CAV", "FBB", "BB", "BBV", "CVL", "CV", "CVB", "SS", "SSV", "AS", "AV", "AO", "AR", "LHA", "DE"],
    }
};