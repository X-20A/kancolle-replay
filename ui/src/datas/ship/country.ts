...
type CountryOriginalDatas = Record<Country, PlayerShipClass[]>;
...
export const COUNTRY_DATAS: CountryDatas = (() => {
    const map: Partial<Record<PlayerShipClass, Country>> = {};

    for (const country of Object.keys(COUNTRY_ORIGINAL_DATAS).map(Number) as Country[]) {
        for (const shipClass of COUNTRY_ORIGINAL_DATAS[country]) {
