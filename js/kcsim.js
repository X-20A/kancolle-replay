 ...
 * dmgMod: ダメージ補正
 * accMod: 命中補正
 * chanceMod: 発動率補正(参照: https://en.kancollewiki.net/w/index.php?title=Combat/Artillery_Spotting&mobileaction=toggle_view_desktop#Trigger_Rates > Trigger rate formula > Base Attack)
 * numHits: 攻撃回数
 * name: カットイン名
 */
 ...
 * @param {number} [attackSpecial] - 特殊攻撃の種類を示す識別子（省略可能）
 * @param {boolean} [combinedAll] - 連合艦隊か（省略可能）。
 ...
 * @param {boolean} NBonly - 夜戦のみか(払暁戦を想定している?)
 ...
 * @param {Array} alive - 生存しているターゲット艦船のリスト
 * @param {Array} subsalive - 生存している潜水艦のリスト
 * @param {boolean} isOASW - 先制対潜攻撃であるか
 * @returns {Object} result - 攻撃対象に関する情報を含むオブジェクト
 */
 ...
 *
 * @param {Ship} ship - 攻撃を行う艦船のオブジェクト
 * @param {Ship} targetData - 攻撃対象
 * @param {Object} APIhou - API用のデータオブジェクト。攻撃結果を保存するために使用される
 * @param {boolean} attackSpecial - 特殊攻撃であるか
 */
 ...
 * @param {boolean} isNB - 夜戦であるかを示すフラグ（true: 夜戦、false: 昼戦）
 * @param {Array} NBequips - 夜戦装備の情報を示す2次元配列
 ...
 * @param {Array<Ship>} targets - 敵艦隊の生存艦情報（主力艦隊と護衛艦隊）
 ...
 * @param {Array<Ship>} targets - 砲撃対象の艦隊情報（生存艦や潜水艦）
 * @param {Object} APIhou - 砲撃戦の結果を記録するAPIデータ
...
/**
 * 特別な攻撃（Hougeki）の結果を調整する
 * 特別な攻撃の回数を反映させるために、攻撃データを適切に調整し、不要なデータを削除します
 *
 ...
 * @returns {[number, number]} - [最終的な命中率, 最終的なクリティカル率]
 */
...
/**
 * 旗艦保護や照明弾による再選択を考慮して目標艦を選択する
 * 目標艦が旗艦の場合、旗艦保護を適用するかを決定
 * 
 * @param {Array<Ship>} targets - 対象艦の配列
 * @param {number | null} searchlightRerolls - 照明弾による再選択回数（0 or null の場合、再選択は行わない）。
...
/**
 * 航空機による防空をシミュレートする関数
 * 航空機の装備が戦闘機（または指定されたフィルター）に該当する場合、対空防御の結果を計算
 * 
 * @param {Array} carriers - 空母艦の配列
 ...
 * 連絡機能が発動すると、触接修正値とその装備IDが返される
 * 触接が失敗した場合は null を返す
 * 
 * @param {Array<Ship>} carriers - 連絡機能を使用する空母艦船の配列
 * @returns {Record<String, number> | null} 触接結果。成功した場合は `{mod: contactMod, id: equip.mid}` の形式で返す。
 *                         失敗した場合は `null` を返す
 */
 ...
 * @param {boolean} isbombing - trueなら自艦隊から敵艦隊へ爆撃を行う
 ...
 * @param {string} doNB - 夜戦するか
 * @param {boolean} NBonly - 夜戦のみか
 * @param {boolean} aironly - 航空戦のみか
 * @param {boolean} bombing - 爆撃フェーズがあるか
 * @param {boolean} noammo - 弾薬がない場合
 * @param {Object} BAPI - APIデータ
 * @param {boolean} noupdate - 結果更新を行わない場合
 ...
 * @param {boolean} NBonly - 夜戦のみの補給か
 * @param {boolean} bombing - 爆撃が行われたか
 * @param {boolean} noammo - 弾薬残量が0であるか
...
function simStats(numsims,foptions) {
    ...
    /**
     * @type {Object} - シミュの結果を格納するオブジェクト
     */
	var totalResult = {
        /** @type {number} - 実行されたシミュレーションの総数 */
		totalnum: numsims,
        /** @type {number} - 総燃料消費 */
		totalFuelS: 0,
        /** @type {number} - 総弾薬消費 */
        ...
        /** @type {number} - 総バケツ消費 */
		totalBuckets: 0,
        /** @type {number} - 総全滅艦載機スロット数 */
		totalEmptiedPlanes: 0,
        /** @type {number} - 総全滅基地航空隊部隊数 */
		totalEmptiedLBAS: 0,
        /** @type {number} - 総削りゲージ量 */
		totalGaugeDamage: 0,
        /** @type {Array<Object>} - 各ノードの結果を格納する配列 */
		nodes: []
	};
    // 各ノードごとの統計情報を初期化
 ...
 * @returns {number|null} - 条件を満たす夜偵装備オブジェクト（条件を満たさない場合はnull）
 */
 ...
 * @returns {[boolean, boolean], [boolean, boolean], [number|null, number|null], [boolean, boolean]]} -    
 * 夜戦装備の状況[自軍, 敵軍]    
