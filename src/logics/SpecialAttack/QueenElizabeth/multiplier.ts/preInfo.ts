...
export type QueenElizabethSpecialMultiplierPreInfo = {
    has_surface_radar: boolean,
    has_AP_shell: boolean,
}
const INITIAL: QueenElizabethSpecialMultiplierPreInfo = {
    has_surface_radar: false,
    has_AP_shell: false,
};

export function calc_QueenElizabeth_special_pre_info(
    equip_slots: PlayerEquipSlot[],
): QueenElizabethSpecialMultiplierPreInfo {
    const pre_info: QueenElizabethSpecialMultiplierPreInfo = equip_slots.reduce((total, slot) => {
        const { equip } = slot;
