import { EquipDatas as PlayerEquipDatas, SpecialIcon } from "@/types/equip/player";
import { EquipImprovementType } from "../improvement";

/** プレイヤー装備種別ID */
export const EQUIP_TYPE = {
    MAIN_GUN_S: 1,
    MAIN_GUN_S_AA: 101,
    MAIN_GUN_M: 2,
    MAIN_GUN_L: 3,
    SECONDARY_GUN: 4,
    SECONDARY_GUN_AA: 104,
    TORPEDO: 5,
    FIGHTER: 6,
    DIVE_BOMBER: 7,
    TORPEDO_BOMBER: 8,
    CARRIER_SCOUT: 9,
    SEAPLANE: 10,
    SEAPLANE_BOMBER: 11,
    RADAR_S: 12,
    RADAR_L: 13,
    SONAR_S: 14,
    /** 爆雷 && 爆雷投射機 */
    DEPTH_CHARGE: 15,
    ENGINE: 17,
    TYPE_3_SHELL: 18,
    AP_SHELL: 19,
    AA_GUN: 21,
    MIDGET_SUBMARINE: 22,
    REPAIR: 23,
    LANDING_CRAFT: 24,
    AUTOGYRO: 25,
    ASW_PLANE: 26,
    BULGE_M: 27,
    BULGE_L: 28,
    SEARCHLIGHT_S: 29,
    DRUM: 30,
    SRF: 31,
    TORPEDO_SS: 32,
    STARSHELL: 33,
    FCF: 34,
    SCAMP: 35,
    ANTI_AIR_FIRE_DIRECTOR: 36,
    WG42: 37,
    MAIN_GUN_XL: 38,
    PICKET: 39,
    SONAR_L: 40,
    FLYING_BOAT: 41,
    SEARCHLIGHT_L: 42,
    RATION: 43,
    OILDRUM: 44,
    SEAPLANE_FIGHTER: 45,
    LANDING_TANK: 46,
    LAND_BASED_BOMBER: 47,
    INTERCEPTOR: 48,
    LAND_BASED_SCOUT: 49,
    LAND_BASED_BOMBER_L: 53,


    TRANSPORT_ITEM: 50,
    SUBMARINE_RADAR: 51,
    ARMY_UNIT: 52,

    SMOKESCREEN: 54,
    JET_BOMBER: 57,
    // JETSCOUT: 59,
    RADAR_XL: 93,
    CARRIER_SCOUT_2: 94,
    SECONDARY_GUN_L: 95,
    OTHER: 99,

    // 新規

    /** 爆戦 */
    FIGHTER_BOMBER: 70,
    /** 阻塞気球 */
    BARRAGE_BALLOON: 79,
} as const;

export type EquipType = keyof typeof EQUIP_TYPE

// 基地航空隊装備も分けるか？ - 現状分けるほどでも無い

export const PLAYER_EQUIP_DATAS: PlayerEquipDatas = {
    1: {
        name: '12cm Single Cannon',
        name_jp: '12cm単装砲',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 1,
        AA: 1,
        RNG: 1
    },
    2: {
        name: '12.7cm Twin Cannon',
        name_jp: '12.7cm連装砲',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 2,
        AA: 2,
        RNG: 1
    },
    3: {
        name: '10cm Twin High-Angle Cannon',
        name_jp: '10cm連装高角砲',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 2,
        AA: 7,
        RNG: 1
    },
    4: {
        name: '14cm Single Cannon',
        name_jp: '14cm単装砲',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        fit_class: 101,
        FP: 2,
        RNG: 2,
        ACC: 1
    },
    5: {
        name: '15.5cm Triple Cannon',
        name_jp: '15.5cm三連装砲',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 7,
        AA: 4,
        RNG: 2,
        ACC: 1
    },
    6: {
        name: '20.3cm Twin Cannon',
        name_jp: '20.3cm連装砲',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 8,
        AA: 3,
        RNG: 2,
        CA_NB_bonus: 10,
    },
    7: {
        name: '35.6cm Twin Cannon',
        name_jp: '35.6cm連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 1,
        FP: 15,
        AA: 4,
        RNG: 3
    },
    8: {
        name: '41cm Twin Cannon',
        name_jp: '41cm連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 3,
        FP: 20,
        AA: 4,
        RNG: 3
    },
    9: {
        name: '46cm Triple Cannon',
        name_jp: '46cm三連装砲',
        type: "MAIN_GUN_L",
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        aaci_type: 'A_XLGUN',
        fit_class: 5,
        FP: 26,
        AA: 5,
        RNG: 4
    },
    10: {
        name: '12.7cm Twin High-Angle Cannon',
        name_jp: '12.7cm連装高角砲',
        type: "SECONDARY_GUN",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 2,
        AA: 4,
        RNG: 1,
        ACC: 1
    },
    11: {
        name: '15.2cm Single Cannon',
        name_jp: '15.2cm単装砲',
        type: "SECONDARY_GUN",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        fit_class: 101,
        FP: 2,
        RNG: 2,
        ACC: 1
    },
    12: {
        name: '15.5cm Triple Secondary Cannon',
        name_jp: '15.5cm三連装副砲',
        type: "SECONDARY_GUN",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        FP: 7,
        AA: 3,
        RNG: 2,
        ACC: 2
    },
    13: {
        name: '61cm Triple Torpedo',
        name_jp: '61cm三連装魚雷',
        type: "TORPEDO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO,
        TP: 5,
        RNG: 1
    },
    14: {
        name: '61cm Quad Torpedo',
        name_jp: '61cm四連装魚雷',
        type: "TORPEDO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO,
        TP: 7,
        RNG: 1
    },
    15: {
        name: '61cm Quad (Oxygen) Torpedo',
        name_jp: '61cm四連装(酸素)魚雷',
        type: "TORPEDO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO,
        TP: 10,
        RNG: 1
    },
    16: {
        name: 'Type 97 Torpedo Bomber',
        name_jp: '九七式艦攻',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AERIAL_TP: 5,
        ASW: 4,
        LOS: 1
    },
    17: {
        name: 'Tenzan',
        name_jp: '天山',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AERIAL_TP: 7,
        ASW: 3,
        LOS: 1
    },
    18: {
        name: 'Ryuusei',
        name_jp: '流星',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AERIAL_TP: 10,
        AA: 1,
        ASW: 4,
        LOS: 1
    },
    19: {
        name: 'Type 96 Fighter',
        name_jp: '九六式艦戦',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 3,
        EV: 1,
    },
    20: {
        name: 'Type 21 Zero Fighter',
        name_jp: '零式艦戦21型',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 5
    },
    21: {
        name: 'Type 52 Zero Fighter',
        name_jp: '零式艦戦52型',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 6
    },
    22: {
        name: 'Prototype Reppuu Late Model',
        name_jp: '試製烈風 後期型',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 10
    },
    23: {
        name: 'Type 99 Bomber',
        name_jp: '九九式艦爆',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        ASW: 3,
        AERIAL_BOMB: 5
    },
    24: {
        name: 'Suisei',
        name_jp: '彗星',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        ASW: 3,
        AERIAL_BOMB: 8
    },
    25: {
        name: 'Type 0 Recon Seaplane',
        name_jp: '零式水上偵察機',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        AA: 1,
        ASW: 2,
        LOS: 5,
        AERIAL_BOMB: 1,
        ACC: 1
    },
    26: {
        name: 'Zuiun',
        name_jp: '瑞雲',
        type: "SEAPLANE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_BOMBER,
        AA: 2,
        ASW: 4,
        LOS: 6,
        AERIAL_BOMB: 4,
        ACC: 1
    },
    27: {
        name: 'Type 13 Air RADAR',
        name_jp: '13号対空電探',
        type: "RADAR_S",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_S_MODEL_A,
        AA: 2,
        LOS: 3,
        ACC: 1
    },
    28: {
        name: 'Type 22 Surface RADAR',
        name_jp: '22号対水上電探',
        type: "RADAR_S",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.RADAR_S_MODEL_B,
        LOS: 5,
        ACC: 3
    },
    29: {
        name: 'Type 33 Surface RADAR',
        name_jp: '33号対水上電探',
        type: "RADAR_S",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.RADAR_S_MODEL_B,
        LOS: 7,
        ACC: 5
    },
    30: {
        name: 'Type 21 Air RADAR',
        name_jp: '21号対空電探',
        type: "RADAR_L",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_L,
        AA: 4,
        LOS: 4,
        ACC: 2
    },
    31: {
        name: 'Type 32 Surface RADAR',
        name_jp: '32号対水上電探',
        type: "RADAR_L",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.RADAR_L,
        LOS: 10,
        ACC: 8
    },
    32: {
        name: 'Type 42 Air RADAR',
        name_jp: '42号対空電探',
        type: "RADAR_L",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_L,
        AA: 6,
        LOS: 5,
        ACC: 4
    },
    33: {
        name: 'Improved Steam Turbine',
        name_jp: '改良型艦本式タービン',
        type: "ENGINE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.ENGINE,
        EV: 6
    },
    34: {
        name: 'Enhanced Steam Boiler',
        name_jp: '強化型艦本式缶',
        type: "ENGINE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.ENGINE,
        EV: 10
    },
    35: {
        name: 'Type 3 Shell',
        name_jp: '三式弾',
        type: "TYPE_3_SHELL",
        aaci_type: 'A_TYPE3SHELL',
        improvement_type: EquipImprovementType.TYPE_3_SHELL,
        AA: 5
    },
    36: {
        name: 'Type 91 AP Shell',
        name_jp: '九一式徹甲弾',
        type: "AP_SHELL",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.AP_SHELL,
        FP: 8,
        ACC: 1
    },
    37: {
        name: '7.7mm Gun',
        name_jp: '7.7mm機銃',
        type: "AA_GUN",
        aaci_type: 'A_AAGUN',
        improvement_type: EquipImprovementType.AA_GUN,
        AA: 2,
        EV: 1
    },
    38: {
        name: '12.7mm Gun',
        name_jp: '12.7mm単装機銃',
        type: "AA_GUN",
        aaci_type: 'A_AAGUN',
        improvement_type: EquipImprovementType.AA_GUN,
        AA: 3,
        EV: 1
    },
    39: {
        name: '25mm Dual Gun',
        name_jp: '25mm連装機銃',
        type: "AA_GUN",
        aaci_type: 'A_AAGUN',
        improvement_type: EquipImprovementType.AA_GUN,
        AA: 5,
        EV: 1
    },
    40: {
        name: '25mm Triple Gun',
        name_jp: '25mm三連装機銃',
        type: "AA_GUN",
        aaci_type: 'A_AAGUN',
        improvement_type: EquipImprovementType.AA_GUN,
        AA: 6,
        EV: 1
    },
    41: {
        name: 'Type A Ko-hyoteki',
        name_jp: '甲標的 甲型',
        type: "MIDGET_SUBMARINE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.MIDGET_SUBMARINE,
        TP: 12
    },
    42: {
        name: 'Repair Team',
        name_jp: '応急修理要員',
        type: "REPAIR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.REPAIR,
    },
    43: {
        name: 'Repair Goddess',
        name_jp: '応急修理女神',
        type: "REPAIR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.REPAIR,
    },
    44: {
        name: 'Type 94 Depth Charge Projector',
        name_jp: '九四式爆雷投射機',
        type: "DEPTH_CHARGE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DEPTH_CHARGE,
        is_DCP: true,
        ASW: 5
    },
    45: {
        name: 'Type 3 Depth Charge Projector',
        name_jp: '三式爆雷投射機',
        type: "DEPTH_CHARGE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DEPTH_CHARGE,
        is_DCP: true,
        ASW: 8
    },
    46: {
        name: 'Type 93 SONAR',
        name_jp: '九三式水中聴音機',
        type: "SONAR_S",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SONAR_S,
        ASW: 6,
        ACC: 1
    },
    47: {
        name: 'Type 3 SONAR',
        name_jp: '三式水中探信儀',
        type: "SONAR_S",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SONAR_S,
        ASW: 10,
        ACC: 2
    },
    48: {
        name: '12cm Single High-Angle Cannon',
        name_jp: '12cm単装高角砲',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 1,
        AA: 3,
        RNG: 1
    },
    49: {
        name: '25mm Single Gun',
        name_jp: '25mm単装機銃',
        type: "AA_GUN",
        aaci_type: 'A_AAGUN',
        improvement_type: EquipImprovementType.AA_GUN,
        AA: 4,
        EV: 1
    },
    50: {
        name: '20.3cm(no.3) Dual Cannon',
        name_jp: '20.3cm(3号)連装砲',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 10,
        AA: 4,
        ACC: 1,
        RNG: 2,
        CA_NB_bonus: 15,
    },
    51: {
        name: '12cm 30-tube Rocket Launcher',
        name_jp: '12cm30連装噴進砲',
        type: "AA_GUN",
        aaci_type: 'A_AAGUN',
        improvement_type: EquipImprovementType.AA_GUN,
        AA: 8
    },
    52: {
        name: 'Ryuusei Kai',
        name_jp: '流星改',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AERIAL_TP: 13,
        AA: 1,
        ASW: 3,
        LOS: 2
    },
    53: {
        name: 'Reppuu Model 11',
        name_jp: '烈風 一一型',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 12
    },
    54: {
        name: 'Saiun',
        name_jp: '彩雲',
        type: "CARRIER_SCOUT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.CARRIER_SCOUT,
        can_avoid_T_disadvantage: true,
        LOS: 9,
        ACC: 2
    },
    55: {
        name: 'Shiden Kai 2',
        name_jp: '紫電改二',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 9,
        EV: 3
    },
    56: {
        name: 'Shinden Kai',
        name_jp: '震電改',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 15
    },
    57: {
        name: 'Suisei Model 12A',
        name_jp: '彗星一二型甲',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        ASW: 3,
        LOS: 1,
        AERIAL_BOMB: 10
    },
    58: {
        name: '61cm Quintuple (Oxygen) Torpedo',
        name_jp: '61cm五連装(酸素)魚雷',
        type: "TORPEDO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO,
        TP: 12,
        RNG: 1,
        ACC: 1
    },
    59: {
        name: 'Type 0 Observation Seaplane',
        name_jp: '零式水上観測機',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        AA: 2,
        ASW: 4,
        LOS: 6,
        AERIAL_BOMB: 1,
        ACC: 2
    },
    60: {
        name: 'Type 62 Zero Fighter-Bomber',
        name_jp: '零式艦戦62型(爆戦)',
        type: "FIGHTER_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER_BOMBER,
        AA: 4,
        ASW: 3,
        AERIAL_BOMB: 4
    },
    61: {
        name: 'Type 2 Recon Plane',
        name_jp: '二式艦上偵察機',
        type: "CARRIER_SCOUT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.CARRIER_SCOUT,
        AA: 1,
        LOS: 7,
        ACC: 3
    },
    62: {
        name: 'Prototype Seiran',
        name_jp: '試製晴嵐',
        type: "SEAPLANE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_BOMBER,
        ASW: 6,
        LOS: 6,
        AERIAL_BOMB: 11,
        ACC: 1
    },
    63: {
        name: '12.7cm Twin Cannon B',
        name_jp: '12.7cm連装砲B型改二',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 3,
        AA: 2,
        RNG: 1
    },
    64: {
        name: 'Ju 87C Kai',
        name_jp: 'Ju87C改',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        can_shell_install_bomber: true,
        ASW: 5,
        AERIAL_BOMB: 9,
        ACC: 1
    },
    65: {
        name: '15.2cm Twin Cannon',
        name_jp: '15.2cm連装砲',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        fit_class: 102,
        FP: 5,
        AA: 3,
        RNG: 2,
        ACC: 3
    },
    66: {
        name: '8cm High-Angle Cannon',
        name_jp: '8cm高角砲',
        type: "SECONDARY_GUN",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 1,
        AA: 6,
        RNG: 1,
        ACC: 2
    },
    67: {
        name: '53cm Hull-mount (Oxygen) Torpedo',
        name_jp: '53cm艦首(酸素)魚雷',
        type: "TORPEDO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO,
        TP: 15,
        RNG: 1,
        ACC: 2
    },
    68: {
        name: 'Daihatsu-Class Landing Craft',
        name_jp: '大発動艇',
        type: "LANDING_CRAFT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LANDING_CRAFT,
    },
    69: {
        name: 'Type Ka Liaison Aircraft',
        name_jp: 'カ号観測機',
        type: "AUTOGYRO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.AUTOGYRO_LOW,
        ASW: 9,
        ACC: 1
    },
    70: {
        name: 'Type 3 Liaison Aircraft',
        name_jp: '三式指揮連絡機(対潜)',
        type: "ASW_PLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.ASW_PLANE_LOW,
        ASW: 7,
        LOS: 1,
        ACC: 2
    },
    71: {
        name: '10cm Twin High-Angle Cannon(Late Model)',
        name_jp: '10cm連装高角砲(砲架)',
        type: "SECONDARY_GUN",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 1,
        AA: 7,
        RNG: 1,
        ACC: 1
    },
    72: {
        name: 'Anti-Torpedo Bulge (M)',
        name_jp: '増設バルジ(中型艦)',
        type: "BULGE_M",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.BULGE_M,
        AR: 7,
        EV: -2
    },
    73: {
        name: 'Anti-Torpedo Bulge (L)',
        name_jp: '増設バルジ(大型艦)',
        type: "BULGE_L",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.BULGE_L,
        AR: 9,
        EV: -3
    },
    74: {
        name: 'Searchlight',
        name_jp: '探照灯',
        type: "SEARCHLIGHT_S",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEARCHLIGHT_S,
        LOS: 2
    },
    75: {
        name: 'Drum (Transport)',
        name_jp: 'ドラム缶(輸送用)',
        type: "DRUM",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DRUM,
    },
    76: {
        name: '38cm Twin Gun Mount',
        name_jp: '38cm連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 1,
        FP: 16,
        AA: 1,
        RNG: 3,
        ACC: 1
    },
    77: {
        name: '15cm Twin Gun Mount',
        name_jp: '15cm連装副砲',
        type: "SECONDARY_GUN",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        FP: 4,
        AA: 2,
        RNG: 2,
        ACC: 2
    },
    78: {
        name: '12.7cm Naval Gun',
        name_jp: '12.7cm単装砲',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 2,
        RNG: 1,
        ACC: 1
    },
    79: {
        name: 'Zuiun(634)',
        name_jp: '瑞雲(六三四空)',
        type: "SEAPLANE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_BOMBER,
        AA_resist_ship: 0.6,
        AA: 2,
        ASW: 5,
        LOS: 6,
        AERIAL_BOMB: 6,
        ACC: 1
    },
    80: {
        name: 'Zuiun Model 12',
        name_jp: '瑞雲12型',
        type: "SEAPLANE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_BOMBER,
        AA_resist_ship: 0.6,
        AA: 3,
        ASW: 5,
        LOS: 6,
        AERIAL_BOMB: 7,
        ACC: 1
    },
    81: {
        name: 'Zuiun Model 12(634)',
        name_jp: '瑞雲12型(六三四空)',
        type: "SEAPLANE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_BOMBER,
        AA_resist_ship: 0.6,
        AA_resist_fleet: .7,
        AA: 3,
        ASW: 6,
        LOS: 7,
        AERIAL_BOMB: 9,
        ACC: 1
    },
    82: {
        name: 'Type 97 Torpedo Bomber(931)',
        name_jp: '九七式艦攻(九三一空)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AERIAL_TP: 6,
        ASW: 7,
        LOS: 2
    },
    83: {
        name: 'Tenzan(931)',
        name_jp: '天山(九三一空)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AERIAL_TP: 9,
        ASW: 8,
        LOS: 2
    },
    84: {
        name: '2cm Flakvierling 38',
        name_jp: '2cm 四連装FlaK 38',
        type: "AA_GUN",
        aaci_type: 'A_AAGUN',
        improvement_type: EquipImprovementType.AA_GUN,
        AA: 7,
        ACC: 1
    },
    85: {
        name: '3.7cm FlaK M42',
        name_jp: '3.7cm FlaK M42',
        type: "AA_GUN",
        aaci_type: 'A_AAGUN',
        improvement_type: EquipImprovementType.AA_GUN,
        FP: 1,
        AA: 8,
        ACC: 1
    },
    86: {
        name: 'Ship Repair Facility',
        name_jp: '艦艇修理施設',
        type: "SRF",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SRF,
    },
    87: {
        name: 'New High Pressure-Temperature Steam Boiler',
        name_jp: '新型高温高圧缶',
        type: "ENGINE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.ENGINE,
        EV: 13
    },
    88: {
        name: 'Type 22 Surface RADAR Kai 4',
        name_jp: '22号対水上電探改四',
        type: "RADAR_S",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.RADAR_S_MODEL_B,
        ASW: 2,
        LOS: 5,
        ACC: 8
    },
    89: {
        name: 'Type 21 Air RADAR Kai',
        name_jp: '21号対空電探改',
        type: "RADAR_L",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_L,
        AA: 5,
        LOS: 6,
        EV: 1,
        ACC: 3
    },
    90: {
        name: '20.3cm(no.2) Twin Gun Mount',
        name_jp: '20.3cm(2号)連装砲',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 9,
        AA: 3,
        RNG: 2,
        ACC: 1,
        CA_NB_bonus: 15,
    },
    91: {
        name: '12.7cm Twin High-Angle Mount (Late Model)',
        name_jp: '12.7cm連装高角砲(後期型)',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 2,
        AA: 5,
        ASW: 1,
        EV: 1,
        RNG: 1,
        ACC: 1
    },
    92: {
        name: 'HI-type 40mm Twin Machine Gun',
        name_jp: '毘式40mm連装機銃',
        type: "AA_GUN",
        aaci_type: 'A_AAGUN',
        improvement_type: EquipImprovementType.AA_GUN,
        AA: 6,
        EV: 1
    },
    93: {
        name: 'Type 97 Torpedo Bomber(Tomonaga)',
        name_jp: '九七式艦攻(友永隊)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_TP: 11,
        AA: 1,
        ASW: 5,
        LOS: 4,
        ACC: 3
    },
    94: {
        name: 'Tenzan Model 12(Tomonaga)',
        name_jp: '天山一二型(友永隊)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_TP: 14,
        AA: 1,
        ASW: 6,
        LOS: 5,
        ACC: 3
    },
    95: {
        name: '53cm Submarine Bow Torpedo Mount (8 tubes)',
        name_jp: '潜水艦53cm艦首魚雷(8門)',
        type: "TORPEDO_SS",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_SS,
        TP: 16,
        RNG: 1,
        ACC: 3
    },
    96: {
        name: 'Type 21 Zero Fighter (Skilled)',
        name_jp: '零式艦戦21型(熟練)',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 8,
        LOS: 1,
        EV: 2,
        ACC: 2
    },
    97: {
        name: 'Type 99 Bomber(Skilled)',
        name_jp: '九九式艦爆(熟練)',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        AA: 1,
        ASW: 4,
        LOS: 2,
        AERIAL_BOMB: 7,
        ACC: 2
    },
    98: {
        name: 'Type 97 Torpedo Bomber(Skilled)',
        name_jp: '九七式艦攻(熟練)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AERIAL_TP: 8,
        ASW: 5,
        LOS: 2,
        ACC: 2
    },
    99: {
        name: 'Type 99 Bomber(Egusa)',
        name_jp: '九九式艦爆(江草隊)',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        AA_resist_ship: 0.6,
        ASW: 5,
        LOS: 3,
        AERIAL_BOMB: 10,
        ACC: 4
    },
    100: {
        name: 'Suisei(Egusa)',
        name_jp: '彗星(江草隊)',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        AA_resist_ship: 0.6,
        AA_resist_fleet: .7,
        AA: 1,
        ASW: 5,
        LOS: 4,
        AERIAL_BOMB: 13,
        ACC: 4
    },
    101: {
        name: 'Star Shell',
        name_jp: '照明弾',
        type: "STARSHELL",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.STARSHELL,
    },
    102: {
        name: 'Type 98 Recon Seaplane (Night Scout)',
        name_jp: '九八式水上偵察機(夜偵)',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        icon: SpecialIcon.NightScout,
        is_night_scout: true,
        ASW: 1,
        LOS: 3,
        ACC: 1
    },
    103: {
        name: 'Prototype 35.6cm Triple Gun Mount',
        name_jp: '試製35.6cm三連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 1,
        FP: 18,
        AA: 5,
        RNG: 3,
        ACC: 2
    },
    104: {
        name: '35.6cm Twin Gun Mount (Dazzle Camouflage)',
        name_jp: '35.6cm連装砲(ダズル迷彩)',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 1,
        FP: 15,
        AA: 5,
        EV: 1,
        RNG: 3,
        ACC: 1
    },
    105: {
        name: 'Prototype 41cm Triple Cannon',
        name_jp: '試製41cm三連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 3,
        FP: 22,
        AA: 5,
        RNG: 3,
        ACC: 2
    },
    106: {
        name: 'Type 13 Air RADAR Kai',
        name_jp: '13号対空電探改',
        type: "RADAR_S",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_S_MODEL_A,
        AA: 4,
        LOS: 4,
        EV: 1,
        ACC: 2
    },
    107: {
        name: 'Fleet Command Facility',
        name_jp: '艦隊司令部施設',
        type: "FCF",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FCF,
        AA: 1,
        LOS: 1,
        EV: 1,
        ACC: 1
    },
    108: {
        name: 'Skilled Carrier-based Aircraft Maintenance Personnel',
        name_jp: '熟練艦載機整備員',
        type: "SCAMP",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SCAMP,
        FP: 10,
        AA: 1,
        LOS: 1,
        RNG: 3,
        ACC: 1
    },
    109: {
        name: 'Zero Fighter Type 52 Type C (601 Air Group)',
        name_jp: '零戦52型丙(六〇一空)',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 9,
        EV: 1,
        ACC: 1
    },
    110: {
        name: 'Reppuu(601)',
        name_jp: '烈風(六〇一空)',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 11,
        EV: 2,
        ACC: 1
    },
    111: {
        name: 'Suisei (601 Air Group)',
        name_jp: '彗星(六〇一空)',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        ASW: 4,
        LOS: 1,
        AERIAL_BOMB: 11,
        ACC: 1
    },
    112: {
        name: 'Tenzan(601)',
        name_jp: '天山(六〇一空)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AERIAL_TP: 10,
        ASW: 4,
        LOS: 2,
        ACC: 1
    },
    113: {
        name: 'Ryuusei(601)',
        name_jp: '流星(六〇一空)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AERIAL_TP: 13,
        ASW: 5,
        LOS: 3,
        ACC: 1
    },
    114: {
        name: '38cm Twin Gun Mount Kai',
        name_jp: '38cm連装砲改',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 1,
        FP: 17,
        AA: 2,
        RNG: 3,
        ACC: 3
    },
    115: {
        name: 'Ar196 Kai',
        name_jp: 'Ar196改',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        AA: 1,
        ASW: 5,
        LOS: 5,
        AERIAL_BOMB: 1,
        ACC: 2
    },
    116: {
        name: 'Type 1 AP Shell',
        name_jp: '一式徹甲弾',
        type: "AP_SHELL",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.AP_SHELL,
        FP: 9,
        ACC: 2
    },
    117: {
        name: 'Prototype 46cm Twin Gun Mount',
        name_jp: '試製46cm連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 4,
        FP: 23,
        AA: 4,
        RNG: 4,
        ACC: 1
    },
    118: {
        name: 'Shiun',
        name_jp: '紫雲',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        ASW: 2,
        LOS: 8,
        AERIAL_BOMB: 1,
        ACC: 1
    },
    119: {
        name: '14cm Twin Gun Mount',
        name_jp: '14cm連装砲',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        fit_class: 102,
        FP: 4,
        RNG: 2,
        ACC: 2
    },
    120: {
        name: 'Type 91 Anti-Aircraft Fire Director',
        name_jp: '91式高射装置',
        type: "ANTI_AIR_FIRE_DIRECTOR",
        aaci_type: 'A_AAFD',
        improvement_type: EquipImprovementType.ANTI_AIR_FIRE_DIRECTOR,
        AA: 2,
        EV: 1
    },
    121: {
        name: 'Type 94 Anti-Aircraft Fire Director',
        name_jp: '94式高射装置',
        type: "ANTI_AIR_FIRE_DIRECTOR",
        aaci_type: 'A_AAFD',
        improvement_type: EquipImprovementType.ANTI_AIR_FIRE_DIRECTOR,
        AA: 3,
        EV: 1
    },
    122: {
        name: '10cm Twin High-Angle Cannon+FD',
        name_jp: '10cm連装高角砲+高射装置',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAFD',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 3,
        AA: 10,
        EV: 1,
        RNG: 1,
        ACC: 1
    },
    123: {
        name: 'SKC34 20.3cm Twin Cannon',
        name_jp: 'SKC34 20.3cm連装砲',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 10,
        AA: 2,
        RNG: 2,
        ACC: 3
    },
    124: {
        name: 'FuMO25 Radar',
        name_jp: 'FuMO25 レーダー',
        type: "RADAR_L",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_L,
        FP: 3,
        AA: 7,
        LOS: 9,
        ACC: 10
    },
    125: {
        name: '61cm Triple (Oxygen) Torpedo Mount',
        name_jp: '61cm三連装(酸素)魚雷',
        type: "TORPEDO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO,
        TP: 8,
        AR: 1,
        EV: 1,
        RNG: 1
    },
    126: {
        name: 'WG42 (Wurfgerat 42)',
        name_jp: 'WG42 (Wurfgerät 42)',
        type: "WG42",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.WG42,
        FP: 1,
        AR: -1,
        RNG: 1
    },
    127: {
        name: 'Prototype FaT Type 95 Oxygen Torpedo Kai',
        name_jp: '試製FaT仕様九五式酸素魚雷改',
        type: "TORPEDO_SS",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_SS,
        TP: 14,
        EV: 2,
        RNG: 1,
        ACC: 7
    },
    128: {
        name: 'Prototype 51cm Twin Cannon',
        name_jp: '試製51cm連装砲',
        type: "MAIN_GUN_XL",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_XL,
        fit_class: 7,
        FP: 30,
        AA: 5,
        EV: -1,
        RNG: 4,
        ACC: 1
    },
    129: {
        name: 'Skilled Lookouts',
        name_jp: '熟練見張員',
        type: "PICKET",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.PICKET,
        AA: 1,
        LOS: 2,
        EV: 3,
        ACC: 2
    },
    130: {
        name: '12.7cm Twin High-angle Mount + Type 94 Anti-Aircraft Fire Director',
        name_jp: '12.7cm高角砲+高射装置',
        type: "SECONDARY_GUN",
        aaci_type: 'A_HAFD',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 1,
        AA: 8,
        EV: 1,
        RNG: 1,
        ACC: 1
    },
    131: {
        name: '25mm Triple Autocannon Mount (Concentrated Deployment)',
        name_jp: '25mm三連装機銃 集中配備',
        type: "AA_GUN",
        aaci_type: 'A_AAGUN',
        improvement_type: EquipImprovementType.AA_GUN,
        is_concentrated: true,
        AA: 9,
        EV: 1
    },
    132: {
        name: 'Type 0 Passive Sonar',
        name_jp: '零式水中聴音機',
        type: "SONAR_L",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SONAR_L,
        ASW: 11,
        LOS: 1,
        EV: 1,
        ACC: 1
    },
    133: {
        name: '381mm / 50 Triple Gun Mount',
        name_jp: '381mm/50 三連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 2,
        FP: 20,
        AA: 2,
        EV: -1,
        RNG: 4,
        ACC: -3
    },
    134: {
        name: 'OTO 152mm Triple Rapid Fire Gun Mount',
        name_jp: 'OTO 152mm三連装速射砲',
        type: "SECONDARY_GUN",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        FP: 8,
        AA: 2,
        AR: 1,
        RNG: 2,
        ACC: 1
    },
    135: {
        name: '90mm Single High-angle Gun Mount',
        name_jp: '90mm単装高角砲',
        type: "SECONDARY_GUN",
        aaci_type: 'A_HAFD',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 1,
        AA: 8,
        RNG: 1,
        ACC: 1
    },
    136: {
        name: 'Pugliese Underwater Protection Bulkhead',
        name_jp: 'プリエーゼ式水中防御隔壁',
        type: "BULGE_L",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.BULGE_L,
        AR: 7,
        EV: -1
    },
    137: {
        name: '381mm/50 Triple Gun Mount Kai',
        name_jp: '381mm/50 三連装砲改',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 2,
        FP: 21,
        AA: 4,
        EV: -1,
        RNG: 4,
        ACC: -1
    },
    138: {
        name: 'Type 2 Large-sized Flying Boat',
        name_jp: '二式大艇',
        type: "FLYING_BOAT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FLYING_BOAT,
        ASW: 1,
        LOS: 12,
        ACC: 1
    },
    139: {
        name: '15.2cm Twin Cannon Kai',
        name_jp: '15.2cm連装砲改',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        fit_class: 102,
        FP: 6,
        AA: 3,
        RNG: 2,
        ACC: 4
    },
    140: {
        name: 'Type 96 150cm Searchlight',
        name_jp: '96式150cm探照灯',
        type: "SEARCHLIGHT_L",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEARCHLIGHT_L,
        AA: 1,
        LOS: 3
    },
    141: {
        name: 'Type 32 Surface RADAR Kai',
        name_jp: '32号対水上電探改',
        type: "RADAR_L",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.RADAR_L,
        LOS: 11,
        ACC: 9
    },
    142: {
        name: '15m Duplex Rangefinder + Type 21 Air Radar Kai2',
        name_jp: '15m二重測距儀+21号電探改二',
        type: "RADAR_XL",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_XL,
        FP: 1,
        AA: 8,
        AR: 1,
        LOS: 7,
        EV: 1,
        ACC: 9
    },
    143: {
        name: 'Type 97 Torpedo Bomber(Murata)',
        name_jp: '九七式艦攻(村田隊)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_TP: 12,
        AA: 1,
        ASW: 5,
        LOS: 4,
        ACC: 2
    },
    144: {
        name: 'Tenzan Model 12(Murata)',
        name_jp: '天山一二型(村田隊)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_TP: 15,
        AA: 1,
        ASW: 6,
        LOS: 4,
        ACC: 2
    },
    145: {
        name: 'Combat Provisions',
        name_jp: '戦闘糧食',
        type: "RATION",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.RATION,
    },
    146: {
        name: 'Underway Replenishment',
        name_jp: '洋上補給',
        type: "OILDRUM",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.OILDRUM,
        AR: -2
    },
    147: {
        name: '120mm/50 Twin Gun Mount',
        name_jp: '120mm/50 連装砲',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 3,
        AA: 2,
        RNG: 1,
        ACC: 1
    },
    148: {
        name: 'Prototype Nanzan',
        name_jp: '試製南山',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        can_shell_install_bomber: true,
        AA: 1,
        ASW: 4,
        LOS: 2,
        AERIAL_BOMB: 11
    },
    149: {
        name: 'Type 4 Passive Sonar',
        name_jp: '四式水中聴音機',
        type: "SONAR_S",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SONAR_S,
        AR: 1,
        ASW: 12,
        ACC: 1
    },
    150: {
        name: 'Canned Saury',
        name_jp: '秋刀魚の缶詰',
        type: "RATION",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.RATION,
    },
    151: {
        name: 'Prototype Keiun',
        name_jp: '試製景雲(艦偵型)',
        type: "CARRIER_SCOUT_2",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.CARRIER_SCOUT_2,
        FP: 2,
        ACC: 2,
        LOS: 11
    },
    152: {
        name: 'Type 52 Zero Fighter (Skilled)',
        name_jp: '零式艦戦52型(熟練)',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 9,
        ACC: 1,
        EV: 2,
        LOS: 1
    },
    153: {
        name: 'Type 52 Model C (Iwai Squadron)',
        name_jp: '零戦52型丙(付岩井小隊)',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 10,
        ACC: 1,
        EV: 2,
        LOS: 1
    },
    154: {
        name: 'Type 62 Zero Fighter-Bomber (Iwai Corps)',
        name_jp: '零戦62型(爆戦/岩井隊)',
        type: "FIGHTER_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER_BOMBER,
        b_type: 'B_NIGHT_BOMBER_DUALROLE',
        AA_resist_ship: 0.6,
        AA_resist_fleet: .7,
        AERIAL_BOMB: 4,
        AA: 7,
        ACC: 1,
        EV: 2,
        ASW: 3,
        LOS: 1
    },
    155: {
        name: 'Type 21 Zero Fighter (Iwamoto Squadron)',
        name_jp: '零戦21型(付岩本小隊)',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 9,
        ACC: 1,
        EV: 3,
        LOS: 1
    },
    156: {
        name: 'Type 52 Model A (Iwamoto Squadron)',
        name_jp: '零戦52型甲(付岩本小隊)',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 11,
        ACC: 1,
        EV: 3,
        LOS: 1
    },
    157: {
        name: 'Type 53 Zero Fighter (Iwamoto Corps)',
        name_jp: '零式艦戦53型(岩本隊)',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 12,
        ACC: 2,
        EV: 4,
        LOS: 3
    },
    158: {
        name: 'Bf 109T Kai',
        name_jp: 'Bf109T改',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 8,
        FP: 1,
        EV: 4
    },
    159: {
        name: 'Fw 190T Kai',
        name_jp: 'Fw190T改',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 10,
        FP: 2,
        EV: 2
    },
    160: {
        name: '10.5cm Twin Gun Mount',
        name_jp: '10.5cm連装砲',
        type: "SECONDARY_GUN",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 3,
        AA: 6,
        ACC: 2,
        EV: 1,
        RNG: 1
    },
    161: {
        name: '16inch Triple Gun Mount Mk.7',
        name_jp: '16inch三連装砲 Mk.7',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 6,
        FP: 24,
        AA: 3,
        AR: 1,
        ACC: 4,
        RNG: 3
    },
    162: {
        name: '203mm/53 Twin Gun Mount',
        name_jp: '203mm/53 連装砲',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 9,
        AA: 1,
        ACC: -2,
        RNG: 3
    },
    163: {
        name: 'Ro.43 Recon Seaplane',
        name_jp: 'Ro.43水偵',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        FP: 1,
        AA: 1,
        ASW: 2,
        ACC: 2,
        EV: 1,
        LOS: 4
    },
    164: {
        name: 'Ro.44 Seaplane Fighter',
        name_jp: 'Ro.44水上戦闘機',
        type: "SEAPLANE_FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_FIGHTER,
        AA: 2,
        ASW: 1,
        ACC: 1,
        EV: 2,
        LOS: 2
    },
    165: {
        name: 'Type 2 Seaplane Fighter Kai',
        name_jp: '二式水戦改',
        type: "SEAPLANE_FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_FIGHTER,
        AA: 3,
        ASW: 1,
        ACC: 1,
        EV: 2,
        LOS: 1
    },
    166: {
        name: 'Daihatsu Landing Craft (Type 89 Medium Tank & Landing Force)',
        name_jp: '大発動艇(八九式中戦車&陸戦隊)',
        type: "LANDING_CRAFT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LANDING_CRAFT,
        b_type: 'B_LC2',
    },
    167: {
        name: 'Special Type 2 Amphibious Landing Craft',
        name_jp: '特二式内火艇',
        type: "LANDING_TANK",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LANDING_TANK,
    },
    168: {
        name: 'Type 96 Land-based Attack Aircraft',
        name_jp: '九六式陸攻',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AERIAL_BOMB: 10,
        AERIAL_TP: 8,
        AA: 1,
        ASW: 2,
        LOS: 2
    },
    169: {
        name: 'Type 1 Land-based Attack Aircraft',
        name_jp: '一式陸攻',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AERIAL_BOMB: 12,
        AERIAL_TP: 10,
        AA: 2,
        ASW: 2,
        LOS: 3
    },
    170: {
        name: 'Type 1 Land-based Attack Aircraft (Nonaka Squadron)',
        name_jp: '一式陸攻(野中隊)',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_BOMB: 13,
        AERIAL_TP: 12,
        AA: 3,
        ASW: 2,
        ACC: 1,
        LOS: 4
    },
    171: {
        name: 'OS2U',
        name_jp: 'OS2U',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        AERIAL_BOMB: 1,
        AA: 1,
        ASW: 1,
        ACC: 2,
        EV: 1,
        LOS: 6
    },
    172: {
        name: '5inch Twin Gun Mount Mk.28 mod.2',
        name_jp: '5inch連装砲 Mk.28 mod.2',
        type: "SECONDARY_GUN",
        aaci_type: 'A_HAFD',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 4,
        AA: 9,
        AR: 1,
        ACC: 2,
        EV: 1,
        RNG: 1,
    },
    173: {
        name: 'Bofors 40mm Quadruple Autocannon Mount',
        name_jp: 'Bofors 40mm四連装機関砲',
        type: "AA_GUN",
        aaci_type: 'A_AAGUN',
        improvement_type: EquipImprovementType.AA_GUN,
        is_concentrated: true,
        FP: 1,
        AA: 11,
        AR: 1,
        ACC: 1,
        EV: 2
    },
    174: {
        name: '53cm Twin Torpedo Mount',
        name_jp: '53cm連装魚雷',
        type: "TORPEDO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO,
        TP: 3,
        EV: 2,
        RNG: 1
    },
    175: {
        name: 'Raiden',
        name_jp: '雷電',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        AA: 6,
        AB: 5,
        IN: 2
    },
    176: {
        name: 'Type 3 Fighter Hien',
        name_jp: '三式戦 飛燕',
        icon: SpecialIcon.ArmyLandBasedFighters,
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        AA: 8,
        AB: 1,
        IN: 3
    },
    177: {
        name: 'Type 3 Fighter Hien (244th Air Combat Group)',
        name_jp: '三式戦 飛燕(飛行第244戦隊)',
        icon: SpecialIcon.ArmyLandBasedFighters,
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        AA: 9,
        AB: 3,
        IN: 4
    },
    178: {
        name: 'PBY-5A Catalina',
        name_jp: 'PBY-5A Catalina',
        type: "FLYING_BOAT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FLYING_BOAT,
        ASW: 2,
        ACC: 2,
        LOS: 9
    },
    179: {
        name: 'Prototype 61cm Sextuple (Oxygen) Torpedo Mount',
        name_jp: '試製61cm六連装(酸素)魚雷',
        type: "TORPEDO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO,
        TP: 14,
        ACC: 2,
        RNG: 1,
    },
    180: {
        name: 'Type 1 Land-based Attack Aircraft Model 22A',
        name_jp: '一式陸攻 二二型甲',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AERIAL_TP: 11,
        AERIAL_BOMB: 12,
        AA: 3,
        ASW: 2,
        LOS: 4,
    },
    181: {
        name: 'Type 0 Fighter Model 32',
        name_jp: '零式艦戦32型',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 5,
        EV: 2,
    },
    182: {
        name: 'Type 0 Fighter Model 32 (Skilled)',
        name_jp: '零式艦戦32型(熟練)',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 8,
        ACC: 1,
        EV: 4,
    },
    183: {
        name: '16inch Triple Gun Mount Mk.7 + GFCS',
        name_jp: '16inch三連装砲 Mk.7+GFCS',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 6,
        FP: 24,
        AA: 3,
        AR: 1,
        ACC: 7,
        EV: 1,
        RNG: 3
    },
    184: {
        name: 'Re.2001 OR Kai',
        name_jp: 'Re.2001 OR改',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 3,
        AA: 6,
        EV: 2,
    },
    185: {
        name: 'Type 3 Fighter Hien Model 1D',
        name_jp: '三式戦 飛燕一型丁',
        icon: SpecialIcon.ArmyLandBasedFighters,
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        AA: 9,
        AB: 2,
        IN: 3,
    },
    186: {
        name: 'Type 1 Land-based Attack Aircraft Model 34',
        name_jp: '一式陸攻 三四型',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AERIAL_BOMB: 12,
        AERIAL_TP: 11,
        AA: 4,
        ASW: 2,
        ACC: 1,
        LOS: 4,
    },
    187: {
        name: 'Ginga',
        name_jp: '銀河',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AERIAL_BOMB: 14,
        AERIAL_TP: 14,
        AA: 3,
        ASW: 3,
        ACC: 1,
        LOS: 3,
    },
    188: {
        name: 'Re.2001 G Kai',
        name_jp: 'Re.2001 G改',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        FP: 3,
        AERIAL_TP: 4,
        AA: 4,
        EV: 2,
    },
    189: {
        name: 'Re.2005 Kai',
        name_jp: 'Re.2005 改',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 1,
        AA: 11,
        EV: 3,
    },
    190: {
        name: '38.1cm Mk.I Twin Gun Mount',
        name_jp: '38.1cm Mk.I連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 1,
        FP: 18,
        AA: 1,
        ACC: 1,
        RNG: 3,
    },
    191: {
        name: 'QF 2-pounder Octuple Pom-pom Gun Mount',
        name_jp: 'QF 2ポンド8連装ポンポン砲',
        type: "AA_GUN",
        aaci_type: 'A_AAGUN',
        improvement_type: EquipImprovementType.AA_GUN,
        is_concentrated: true,
        FP: 1,
        AA: 10,
    },
    192: {
        name: '38.1cm Mk.I/N Twin Gun Mount Kai',
        name_jp: '38.1cm Mk.I/N連装砲改',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 1,
        FP: 19,
        AA: 3,
        ACC: 2,
        RNG: 3,
    },
    193: {
        name: 'Toku Daihatsu Landing Craft',
        name_jp: '特大発動艇',
        type: "LANDING_CRAFT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LANDING_CRAFT,
    },
    194: {
        name: 'Laté 298B',
        name_jp: 'Laté 298B',
        type: "SEAPLANE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_BOMBER,
        FP: 2,
        AERIAL_BOMB: 5,
        TP: 4,
        AA: 1,
        ASW: 2,
        EV: 1,
        LOS: 4,
    },
    195: {
        name: 'SBD',
        name_jp: 'SBD',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        FP: 1,
        AERIAL_BOMB: 6,
        AA: 2,
        ASW: 3,
        ACC: 1,
        EV: 2,
        LOS: 2,
    },
    196: {
        name: 'TBD',
        name_jp: 'TBD',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        FP: 1,
        AERIAL_TP: 5,
        ASW: 4,
        LOS: 2,
    },
    197: {
        name: 'F4F-3',
        name_jp: 'F4F-3',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 1,
        AA: 4,
        EV: 2,
    },
    198: {
        name: 'F4F-4',
        name_jp: 'F4F-4',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 1,
        AA: 5,
        ACC: 1,
        EV: 2,
        LOS: 1,
    },
    199: {
        name: 'Jet Keiun Kai',
        name_jp: '噴式景雲改',
        type: "JET_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.JET_BOMBER,
        AA_resist_ship: .5,
        AA_resist_fleet: .7,
        AERIAL_BOMB: 15,
        AA: 6,
        ACC: 1,
        LOS: 3,
        RNG: 3,
    },
    200: {
        name: 'Kikka Kai',
        name_jp: '橘花改',
        type: "JET_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.JET_BOMBER,
        icon: SpecialIcon.KikkaKai,
        AA_resist_ship: .5,
        AA_resist_fleet: .5,
        AERIAL_BOMB: 11,
        AA: 12,
        EV: 1,
        RNG: 3,
    },
    201: {
        name: 'Shiden Model 11',
        name_jp: '紫電一一型',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        AA: 8,
        AR: 1,
        AB: 1,
        IN: 1,
    },
    202: {
        name: 'Shiden Model 21 Shiden Kai',
        name_jp: '紫電二一型 紫電改',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        AA: 9,
        AR: 2,
        AB: 1,
        IN: 3,
    },
    203: {
        name: 'New Kanhon Design Anti-Torpedo Bulge (M)',
        name_jp: '艦本新設計 増設バルジ(中型艦)',
        type: "BULGE_M",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.BULGE_M,
        AR: 8,
        EV: -1,
    },
    204: {
        name: 'New Kanhon Design Anti-Torpedo Bulge (L)',
        name_jp: '艦本新設計 増設バルジ(大型艦)',
        type: "BULGE_L",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.BULGE_L,
        AR: 10,
        EV: -2,
    },
    205: {
        name: 'F6F-3',
        name_jp: 'F6F-3',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 1,
        AA: 8,
        ACC: 1,
        EV: 2,
        LOS: 1,
    },
    206: {
        name: 'F6F-5',
        name_jp: 'F6F-5',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 1,
        AA: 10,
        ACC: 1,
        EV: 3,
        LOS: 1,
    },
    207: {
        name: 'Zuiun (631 Air Group)',
        name_jp: '瑞雲(六三一空)',
        type: "SEAPLANE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_BOMBER,
        AERIAL_BOMB: 7,
        AA: 1,
        ASW: 2,
        ACC: 2,
        LOS: 4,
    },
    208: {
        name: 'Seiran (631 Air Group)',
        name_jp: '晴嵐(六三一空)',
        type: "SEAPLANE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_BOMB: 12,
        ASW: 3,
        ACC: 2,
        LOS: 6,
    },
    209: {
        name: 'Saiun (Disassembled for Transport)',
        name_jp: '彩雲(輸送用分解済)',
        type: "TRANSPORT_ITEM",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TRANSPORT_ITEM,
    },
    210: {
        name: 'Submarine Radar & Waterproof Telescope',
        name_jp: '潜水艦搭載電探&水防式望遠鏡',
        type: "SUBMARINE_RADAR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SUBMARINE_RADAR,
        FP: 1,
        AA: 1,
        ACC: 2,
        EV: 8,
        LOS: 4,
    },
    211: {
        name: 'Submarine Radar & Passive Radiolocator (E27)',
        name_jp: '潜水艦搭載電探&逆探(E27)',
        type: "SUBMARINE_RADAR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SUBMARINE_RADAR,
        FP: 1,
        AA: 1,
        ACC: 3,
        EV: 11,
        LOS: 5,
    },
    212: {
        name: 'Saiun (Eastern Caroline Air Group)',
        name_jp: '彩雲(東カロリン空)',
        type: "CARRIER_SCOUT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.CARRIER_SCOUT,
        can_avoid_T_disadvantage: true,
        FP: 1,
        ACC: 2,
        LOS: 10,
    },
    213: {
        name: 'Late Model Bow Torpedo Mount (6 tubes)',
        name_jp: '後期型艦首魚雷(6門)',
        type: "TORPEDO_SS",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_SS,
        is_special_submarine_CI_torigger: true,
        TP: 15,
        ACC: 3,
        EV: 1,
        RNG: 1,
    },
    214: {
        name: 'Skilled Sonar Personnel + Late Model Bow Torpedo Mount (6 tubes)',
        name_jp: '熟練聴音員+後期型艦首魚雷(6門)',
        type: "TORPEDO_SS",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_SS,
        is_special_submarine_CI_torigger: true,
        TP: 15,
        ACC: 5,
        EV: 4,
        LOS: 1,
        RNG: 1,
    },
    215: {
        name: 'Ro.44 Seaplane Fighter bis',
        name_jp: 'Ro.44水上戦闘機bis',
        type: "SEAPLANE_FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_FIGHTER,
        AA: 3,
        ASW: 1,
        ACC: 2,
        EV: 2,
        LOS: 3,
    },
    216: {
        name: 'Type 2 Seaplane Fighter Kai (Skilled)',
        name_jp: '二式水戦改(熟練)',
        type: "SEAPLANE_FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_FIGHTER,
        AA: 5,
        ASW: 1,
        ACC: 1,
        EV: 2,
        LOS: 1,
    },
    217: {
        name: 'Kyoufuu Kai',
        name_jp: '強風改',
        type: "SEAPLANE_FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_FIGHTER,
        AA: 5,
        EV: 3,
        LOS: 1,
    },
    218: {
        name: 'Type 4 Fighter Hayate',
        name_jp: '四式戦 疾風',
        icon: SpecialIcon.ArmyLandBasedFighters,
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        AA: 10,
        AB: 1,
        IN: 1,
    },
    219: {
        name: 'Type 0 Fighter Model 63 (Fighter-bomber)',
        name_jp: '零式艦戦63型(爆戦)',
        type: "FIGHTER_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER_BOMBER,
        AERIAL_BOMB: 5,
        AA: 5,
        ASW: 2,
    },
    220: {
        name: '8cm High-angle Gun Kai + Extra Machine Guns',
        name_jp: '8cm高角砲改+増設機銃',
        type: "SECONDARY_GUN",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 1,
        AA: 7,
        ACC: 2,
        EV: 1,
        RNG: 1,
    },
    221: {
        name: 'Type 1 Fighter Hayabusa Model II',
        name_jp: '一式戦 隼II型',
        icon: SpecialIcon.ArmyLandBasedFighters,
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        AA: 6,
        IN: 2,
    },
    222: {
        name: 'Type 1 Fighter Hayabusa Model III A',
        name_jp: '一式戦 隼III型甲',
        icon: SpecialIcon.ArmyLandBasedFighters,
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        FP: 1,
        AA: 7,
        AB: 1,
        IN: 3,
    },
    223: {
        name: 'Type 1 Fighter Hayabusa Model III A (54th Squadron)',
        name_jp: '一式戦 隼III型甲(54戦隊)',
        icon: SpecialIcon.ArmyLandBasedFighters,
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        FP: 2,
        AA: 8,
        AB: 1,
        IN: 3,
        LOS: 1,
    },
    224: {
        name: 'Bomb-carrying Type 1 Fighter Hayabusa Model III Kai (65th Squadron)',
        name_jp: '爆装一式戦 隼III型改(65戦隊)',
        icon: SpecialIcon.ArmyLandBasedFighters,
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AA_resist_ship: 0.6,
        AA_resist_fleet: .7,
        FP: 3,
        AERIAL_BOMB: 9,
        AA: 6,
        ACC: 4,
        EV: 2,
        LOS: 1,
    },
    225: {
        name: 'Type 1 Fighter Hayabusa Model II (64th Squadron)',
        name_jp: '一式戦 隼II型(64戦隊)',
        icon: SpecialIcon.ArmyLandBasedFighters,
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        FP: 1,
        AA: 11,
        AB: 1,
        IN: 5,
        LOS: 1,
    },
    226: {
        name: 'Type 95 Depth Charge',
        name_jp: '九五式爆雷',
        type: "DEPTH_CHARGE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DEPTH_CHARGE,
        is_DC_only: true,
        can_ASW_penetrate: true,
        ASW: 4,
    },
    227: {
        name: 'Type 2 Depth Charge',
        name_jp: '二式爆雷',
        type: "DEPTH_CHARGE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DEPTH_CHARGE,
        is_DC_only: true,
        can_ASW_penetrate: true,
        ASW: 7,
    },
    228: {
        name: 'Type 96 Fighter Kai',
        name_jp: '九六式艦戦改',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 4,
        EV: 2,
    },
    229: {
        name: '12.7cm Single High-Angle Gun Mount (Late Model)',
        name_jp: '12.7cm単装高角砲(後期型)',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 1,
        AA: 4,
        ACC: 1,
        EV: 1,
        ASW: 1,
        RNG: 1,
    },
    230: {
        name: 'Toku Daihatsu Landing Craft + 11th Tank Regiment',
        name_jp: '特大発動艇+戦車第11連隊',
        type: "LANDING_CRAFT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LANDING_CRAFT,
    },
    231: {
        name: '30.5cm Triple Gun Mount',
        name_jp: '30.5cm三連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 8,
        FP: 16,
        AR: 1,
        ACC: 1,
        RNG: 3,
    },
    232: {
        name: '30.5cm Triple Gun Mount Kai',
        name_jp: '30.5cm三連装砲改',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 8,
        FP: 17,
        AA: 3,
        AR: 1,
        ACC: 3,
        EV: 1,
        RNG: 3,
    },
    233: {
        name: 'F4U-1D',
        name_jp: 'F4U-1D',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        can_shell_install_bomber: true,
        FP: 1,
        AERIAL_BOMB: 7,
        AA: 7,
        EV: 1,
        ASW: 1,
        LOS: 1,
    },
    234: {
        name: '15.5cm Triple Secondary Gun Mount Kai',
        name_jp: '15.5cm三連装副砲改',
        type: "SECONDARY_GUN",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        FP: 7,
        AA: 5,
        ACC: 4,
        EV: 1,
        RNG: 2,
    },
    235: {
        name: '15.5cm Triple Gun Mount Kai',
        name_jp: '15.5cm三連装砲改',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 7,
        AA: 5,
        AR: 1,
        ACC: 3,
        EV: 1,
        RNG: 2,
    },
    236: {
        name: '41cm Triple Gun Mount Kai',
        name_jp: '41cm三連装砲改',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 3,
        FP: 22,
        AA: 6,
        AR: 1,
        ACC: 4,
        RNG: 3,
    },
    237: {
        name: 'Zuiun (634 Air Group/Skilled)',
        name_jp: '瑞雲(六三四空/熟練)',
        type: "SEAPLANE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_BOMBER,
        AA_resist_ship: 0.6,
        AA_resist_fleet: .7,
        AERIAL_BOMB: 9,
        AA: 4,
        ACC: 1,
        EV: 1,
        ASW: 6,
        LOS: 7,
    },
    238: {
        name: 'Type 0 Reconnaissance Seaplane Model 11B',
        name_jp: '零式水上偵察機11型乙',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        FP: 1,
        AA: 1,
        ACC: 2,
        ASW: 7,
        LOS: 6,
    },
    239: {
        name: 'Type 0 Reconnaissance Seaplane Model 11B (Skilled)',
        name_jp: '零式水上偵察機11型乙(熟練)',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        FP: 2,
        AA: 1,
        ACC: 3,
        ASW: 8,
        LOS: 8,
    },
    240: {
        name: 'Type 22 Surface Radar Kai 4 (Calibrated Late Model)',
        name_jp: '22号対水上電探改四(後期調整型)',
        type: "RADAR_S",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.RADAR_S_MODEL_D,
        FP: 1,
        ACC: 9,
        ASW: 2,
        LOS: 7,
    },
    241: {
        name: 'Combat Ration (Special Onigiri)',
        name_jp: '戦闘糧食(特別なおにぎり)',
        type: "RATION",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.RATION,
    },
    242: {
        name: 'Swordfish',
        name_jp: 'Swordfish',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        b_type: 'B_NIGHT_BOMBER_DUALROLE',
        is_Swordfish_family: true,
        FP: 2,
        AERIAL_TP: 3,
        ACC: 1,
        ASW: 4,
        LOS: 1,
    },
    243: {
        name: 'Swordfish Mk.II(Skilled)',
        name_jp: 'Swordfish Mk.II(熟練)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        b_type: 'B_NIGHT_BOMBER_DUALROLE',
        is_Swordfish_family: true,
        FP: 3,
        AERIAL_TP: 5,
        ACC: 3,
        ASW: 6,
        LOS: 2,
    },
    244: {
        name: 'Swordfish Mk.III (Skilled)',
        name_jp: 'Swordfish Mk.III(熟練)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        b_type: 'B_NIGHT_BOMBER_DUALROLE',
        is_Swordfish_family: true,
        FP: 4,
        AERIAL_TP: 8,
        ACC: 4,
        ASW: 10,
        LOS: 5,
    },
    245: {
        name: '38cm Quadruple Gun Mount',
        name_jp: '38cm四連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 9,
        FP: 21,
        AA: 1,
        ACC: 1,
        RNG: 3,
    },
    246: {
        name: '38cm Quadruple Gun Mount Kai',
        name_jp: '38cm四連装砲改',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 9,
        FP: 22,
        AA: 2,
        AR: 1,
        ACC: 3,
        RNG: 2,
    },
    247: {
        name: '15.2cm Triple Gun Mount',
        name_jp: '15.2cm三連装砲',
        type: "SECONDARY_GUN",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        FP: 6,
        AA: 1,
        ACC: 3,
        RNG: 2,
    },
    248: {
        name: 'Skua',
        name_jp: 'Skua',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        AERIAL_BOMB: 4,
        AA: 2,
        EV: 2,
        ASW: 2,
    },
    249: {
        name: 'Fulmar',
        name_jp: 'Fulmar',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 1,
        AA: 3,
        ACC: 2,
        EV: 1,
        ASW: 2,
        LOS: 1,
    },
    250: {
        name: 'Spitfire Mk.I',
        name_jp: 'Spitfire Mk.I',
        icon: SpecialIcon.ArmyLandBasedFighters,
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        FP: 1,
        AA: 7,
        AB: 2,
        IN: 1,
    },
    251: {
        name: 'Spitfire Mk.V',
        name_jp: 'Spitfire Mk.V',
        icon: SpecialIcon.ArmyLandBasedFighters,
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        FP: 1,
        AA: 9,
        AB: 3,
        IN: 2,
    },
    252: {
        name: 'Seafire Mk.III Kai',
        name_jp: 'Seafire Mk.III改',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 1,
        AA: 9,
        AR: 1,
        ACC: 1,
        EV: 2,
    },
    253: {
        name: 'Spitfire Mk.IX (Skilled)',
        name_jp: 'Spitfire Mk.IX(熟練)',
        icon: SpecialIcon.ArmyLandBasedFighters,
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        FP: 1,
        AA: 10,
        AB: 2,
        IN: 4,
    },
    254: {
        name: 'F6F-3N',
        name_jp: 'F6F-3N',
        icon: SpecialIcon.NightFighters,
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        b_type: 'B_NIGHT_FIGHTER',
        AA: 8,
        ACC: 2,
        EV: 3,
        ASW: 4,
        LOS: 2,
    },
    255: {
        name: 'F6F-5N',
        name_jp: 'F6F-5N',
        icon: SpecialIcon.NightFighters,
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        b_type: 'B_NIGHT_FIGHTER',
        AA: 10,
        ACC: 3,
        EV: 3,
        ASW: 5,
        LOS: 3,
    },
    256: {
        name: 'TBF',
        name_jp: 'TBF',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        FP: 2,
        AERIAL_TP: 9,
        AA: 1,
        ASW: 6,
        LOS: 2,
    },
    257: {
        name: 'TBM-3D',
        name_jp: 'TBM-3D',
        icon: SpecialIcon.NightTorpedoBombers,
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        b_type: 'B_NIGHT_TORPEDO_BOMBER',
        FP: 2,
        AERIAL_TP: 9,
        AA: 1,
        ACC: 2,
        ASW: 8,
        LOS: 4,
    },
    258: {
        name: 'Night Battle Operation Aviation Personnel',
        name_jp: '夜間作戦航空要員',
        type: "SCAMP",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SCAMP,
        b_type: 'B_NIGHT_CREW',
        FP: 1,
        AR: 1,
        RNG: 2,
    },
    259: {
        name: 'Night Battle Operation Aviation Personnel + Skilled Deck Hands',
        name_jp: '夜間作戦航空要員+熟練甲板員',
        type: "SCAMP",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SCAMP,
        b_type: 'B_NIGHT_CREW',
        FP: 3,
        AR: 1,
        ACC: 2,
        EV: 1,
        RNG: 3,
    },
    260: {
        name: 'Type124 ASDIC',
        name_jp: 'Type124 ASDIC',
        type: "SONAR_S",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SONAR_S,
        ACC: 2,
        EV: 1,
        ASW: 11,
    },
    261: {
        name: 'Type144/147 ASDIC',
        name_jp: 'Type144/147 ASDIC',
        type: "SONAR_S",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SONAR_S,
        ACC: 3,
        EV: 1,
        ASW: 13,
    },
    262: {
        name: 'HF/DF + Type144/147 ASDIC',
        name_jp: 'HF/DF + Type144/147 ASDIC',
        type: "SONAR_S",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SONAR_S,
        ACC: 3,
        EV: 2,
        ASW: 15,
        LOS: 2,
    },
    263: {
        name: 'Shiden Kai (343rd Air Group) 301st Fighter Squadron',
        name_jp: '紫電改(三四三空) 戦闘301',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        AA: 11,
        AR: 2,
        AB: 2,
        IN: 4,
    },
    264: {
        name: 'Shiden Kai (343 Air Group) 701st Fighter Squadron',
        name_jp: '紫電改(三四三空) 戦闘701',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        AA: 11,
        AR: 2,
        AB: 2,
        IN: 4,
    },
    265: {
        name: 'Shiden Kai (343 Air Group) 407th Fighter Squadron',
        name_jp: '紫電改(三四三空) 戦闘407',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        AA: 11,
        AR: 2,
        AB: 2,
        IN: 4,
    },
    266: {
        name: '12.7cm Twin Gun Mount Model C Kai 2',
        name_jp: '12.7cm連装砲C型改二',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 3,
        AA: 2,
        AR: 1,
        ACC: 1,
        RNG: 1,
    },
    267: {
        name: '12.7cm Twin Gun Mount Model D Kai 2',
        name_jp: '12.7cm連装砲D型改二',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 3,
        AA: 4,
        AR: 1,
        ACC: 2,
        EV: 1,
        RNG: 1,
    },
    268: {
        name: 'Arctic Camouflage (+ Arctic Equipment)',
        name_jp: '北方迷彩(+北方装備)',
        type: "BULGE_M",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.BULGE_M,
        AR: 2,
        EV: 2,
    },
    269: {
        name: 'Prototype Toukai',
        name_jp: '試製東海',
        icon: SpecialIcon.LandBasedPatrolAircraft,
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AERIAL_BOMB: 2,
        ASW: 10,
        LOS: 5,
    },
    270: {
        name: 'Toukai (901 Air Group)',
        name_jp: '東海(九〇一空)',
        icon: SpecialIcon.LandBasedPatrolAircraft,
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AERIAL_BOMB: 2,
        ACC: 1,
        ASW: 11,
        LOS: 6,
    },
    271: {
        name: 'Shiden Kai 4',
        name_jp: '紫電改四',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 10,
        AR: 1,
        ACC: 1,
        EV: 3,
    },
    272: {
        name: 'Striking Force Fleet Command Facility',
        name_jp: '遊撃部隊 艦隊司令部',
        type: "FCF",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FCF,
        AR: 1,
        ACC: 1,
        EV: 1,
        LOS: 1,
    },
    273: {
        name: 'Saiun (4th Recon)',
        name_jp: '彩雲(偵四)',
        type: "CARRIER_SCOUT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.CARRIER_SCOUT,
        can_avoid_T_disadvantage: true,
        AA: 2,
        ACC: 2,
        LOS: 10,
    },
    274: {
        name: '12cm 30-tube Rocket Launcher Kai Ni',
        name_jp: '12cm30連装噴進砲改二',
        type: "AA_GUN",
        aaci_type: 'A_AAGUN',
        improvement_type: EquipImprovementType.AA_GUN,
        can_barrage: true,
        AA: 8,
        AR: 1,
        ACC: 1,
        EV: 3,
    },
    275: {
        name: '10cm Twin High-angle Gun Mount Kai + Additional Machine Guns',
        name_jp: '10cm連装高角砲改+増設機銃',
        type: "SECONDARY_GUN",
        aaci_type: 'A_HAFD',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 2,
        AA: 9,
        AR: 1,
        ACC: 2,
        EV: 1,
        RNG: 1,
    },
    276: {
        name: '46cm Triple Gun Mount Kai',
        name_jp: '46cm三連装砲改',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 10,
        FP: 27,
        AA: 6,
        AR: 2,
        ACC: 2,
        RNG: 4,
    },
    277: {
        name: 'FM-2',
        name_jp: 'FM-2',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        can_shell_install_bomber: true,
        FP: 2,
        AERIAL_BOMB: 2,
        AA: 6,
        ACC: 2,
        EV: 2,
        ASW: 1,
    },
    278: {
        name: 'SK Radar',
        name_jp: 'SK レーダー',
        type: "RADAR_L",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_L,
        AA: 8,
        ACC: 1,
        EV: 2,
        LOS: 10,
    },
    279: {
        name: 'SK+SG Radar',
        name_jp: 'SK+SG レーダー',
        type: "RADAR_L",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_L,
        FP: 1,
        AA: 9,
        ACC: 4,
        EV: 4,
        LOS: 12,
        RNG: 2,
    },
    280: {
        name: 'QF 4.7inch Gun Mk.XII Kai',
        name_jp: 'QF 4.7inch砲 Mk.XII改',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 3,
        AA: 3,
        ACC: 1,
        EV: 1,
        ASW: 1,
        RNG: 1,
    },
    281: {
        name: '51cm Twin Gun Mount',
        name_jp: '51cm連装砲',
        type: "MAIN_GUN_XL",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_XL,
        FP: 32,
        AA: 5,
        AR: 2,
        ACC: 1,
        EV: -1,
        RNG: 4,
    },
    282: {
        name: '130mm B-13 Twin Gun Mount',
        name_jp: '130mm B-13連装砲',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 4,
        AA: 1,
        AR: 1,
        RNG: 1,
    },
    283: {
        name: '533mm Triple Torpedo Mount',
        name_jp: '533mm 三連装魚雷',
        type: "TORPEDO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO,
        FP: 1,
        TP: 5,
        EV: 1,
        RNG: 1,
    },
    284: {
        name: '5inch Single Gun Mount Mk.30',
        name_jp: '5inch単装砲 Mk.30',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 2,
        AA: 6,
        AR: 1,
        ACC: 1,
        EV: 2,
        RNG: 1,
    },
    285: {
        name: '61cm Triple (Oxygen) Torpedo Mount Late Model',
        name_jp: '61cm三連装(酸素)魚雷後期型',
        type: "TORPEDO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO,
        TP: 9,
        AR: 1,
        ACC: 2,
        EV: 2,
        RNG: 1,
    },
    286: {
        name: '61cm Quadruple (Oxygen) Torpedo Mount Late Model',
        name_jp: '61cm四連装(酸素)魚雷後期型',
        type: "TORPEDO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO,
        TP: 11,
        AR: 1,
        ACC: 2,
        RNG: 1,
    },
    287: {
        name: 'Type 3 Depth Charge Projector (Concentrated Deployment)',
        name_jp: '三式爆雷投射機 集中配備',
        type: "DEPTH_CHARGE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DEPTH_CHARGE,
        is_DCP: true,
        ASW: 12,
    },
    288: {
        name: 'Prototype 15cm 9-tube ASW Rocket Launcher',
        name_jp: '試製15cm9連装対潜噴進砲',
        type: "DEPTH_CHARGE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DEPTH_CHARGE,
        is_DCP: true,
        ACC: 1,
        ASW: 15,
    },
    289: {
        name: '35.6cm Triple Gun Mount Kai (Dazzle Camouflage)',
        name_jp: '35.6cm三連装砲改(ダズル迷彩仕様)',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 1,
        FP: 19,
        AA: 5,
        ACC: 3,
        EV: 2,
        RNG: 3,
    },
    290: {
        name: '41cm Triple Gun Mount Kai Ni',
        name_jp: '41cm三連装砲改二',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 3,
        FP: 23,
        AA: 6,
        ACC: 5,
        RNG: 3,
    },
    291: {
        name: 'Suisei Model 22 (634 Air Group)',
        name_jp: '彗星二二型(六三四空)',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        AERIAL_BOMB: 11,
        AA: 1,
        ACC: 2,
        ASW: 4,
        RNG: 3,
    },
    292: {
        name: 'Suisei Model 22 (634 Air Group / Skilled)',
        name_jp: '彗星二二型(六三四空/熟練)',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        AERIAL_BOMB: 12,
        AA: 2,
        ACC: 3,
        ASW: 5,
        LOS: 2,
        RNG: 3,
    },
    293: {
        name: '12cm Single Gun Mount Kai Ni',
        name_jp: '12cm単装砲改二',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 1,
        AR: 1,
        ACC: 1,
        EV: 1,
        RNG: 1,
    },
    294: {
        name: '12.7cm Twin Gun Mount Model A Kai Ni',
        name_jp: '12.7cm連装砲A型改二',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 2,
        AA: 1,
        AR: 1,
        ACC: 1,
        RNG: 1,
    },
    295: {
        name: '12.7cm Twin Gun Mount Model A Kai 3 (Wartime Modification) + Anti-Aircraft Fire Director',
        name_jp: '12.7cm連装砲A型改三(戦時改修)+高射装置',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAFD',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 2,
        AA: 8,
        AR: 1,
        ACC: 1,
        RNG: 1,
    },
    296: {
        name: '12.7cm Twin Gun Mount Model B Kai 4 (Wartime Modification) + Anti-Aircraft Fire Director',
        name_jp: '12.7cm連装砲B型改四(戦時改修)+高射装置',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAFD',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 3,
        AA: 9,
        AR: 1,
        ACC: 1,
        RNG: 1,
    },
    297: {
        name: '12.7cm Twin Gun Mount Model A',
        name_jp: '12.7cm連装砲A型',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 2,
        AA: 1,
        RNG: 1,
    },
    298: {
        name: '16inch Mk.I Triple Gun Mount',
        name_jp: '16inch Mk.I三連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 11,
        FP: 21,
        AR: 1,
        ACC: 2,
        RNG: 3,
    },
    299: {
        name: '16inch Mk.I Triple Gun Mount + AFCT Kai',
        name_jp: '16inch Mk.I三連装砲+AFCT改',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 11,
        FP: 22,
        AR: 1,
        ACC: 4,
        RNG: 3,
    },
    300: {
        name: '16inch Mk.I Triple Gun Mount Kai + FCR Type 284',
        name_jp: '16inch Mk.I三連装砲改+FCR type284',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 11,
        FP: 23,
        AA: 7,
        AR: 1,
        ACC: 6,
        EV: 2,
        RNG: 3,
    },
    301: {
        name: '20-tube 7inch UP Rocket Launchers',
        name_jp: '20連装7inch UP Rocket Launchers',
        type: "AA_GUN",
        aaci_type: 'A_AAGUN',
        improvement_type: EquipImprovementType.AA_GUN,
        AA: 7,
        EV: 2,
    },
    302: {
        name: 'Type 97 Torpedo Bomber (931 Air Group / Skilled)',
        name_jp: '九七式艦攻(九三一空/熟練)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AERIAL_TP: 8,
        ACC: 2,
        EV: 1,
        ASW: 9,
        LOS: 3,
    },
    303: {
        name: 'Bofors 15.2cm Twin Gun Mount Model 1930',
        name_jp: 'Bofors 15.2cm連装砲 Model 1930',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        fit_class: 102,
        FP: 5,
        AA: 4,
        ACC: 3,
        EV: 1,
        RNG: 2,
    },
    304: {
        name: 'S9 Osprey',
        name_jp: 'S9 Osprey',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        AA: 1,
        ACC: 2,
        ASW: 2,
        LOS: 4,
    },
    305: {
        name: 'Ju 87C Kai Ni (w/ KMX)',
        name_jp: 'Ju87C改二(KMX搭載機)',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        can_shell_install_bomber: true,
        AERIAL_BOMB: 9,
        ACC: 2,
        EV: 1,
        ASW: 9,
        LOS: 2,
    },
    306: {
        name: 'Ju 87C Kai Ni (w/ KMX / Skilled)',
        name_jp: 'Ju87C改二(KMX搭載機/熟練)',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        can_shell_install_bomber: true,
        AERIAL_BOMB: 10,
        ACC: 3,
        EV: 1,
        ASW: 10,
        LOS: 2,
        RNG: 3,
    },
    307: {
        name: 'GFCS Mk.37',
        name_jp: 'GFCS Mk.37',
        type: "RADAR_S",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_S_MODEL_A,
        FP: 2,
        AA: 7,
        AR: 1,
        ACC: 9,
        EV: 2,
        LOS: 6,
        RNG: 2,
    },
    308: {
        name: '5inch Single Gun Mount Mk.30 Kai + GFCS Mk.37',
        name_jp: '5inch単装砲 Mk.30改+GFCS Mk.37',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAFD',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 3,
        AA: 11,
        AR: 1,
        ACC: 6,
        EV: 2,
        LOS: 3,
        RNG: 2,
    },
    309: {
        name: 'Type C Kouhyouteki',
        name_jp: '甲標的 丙型',
        type: "MIDGET_SUBMARINE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.MIDGET_SUBMARINE,
        TP: 14,
        ACC: 1,
        EV: 1,
    },
    310: {
        name: '14cm Twin Gun Mount Kai',
        name_jp: '14cm連装砲改',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        fit_class: 102,
        FP: 5,
        AR: 1,
        ACC: 3,
        RNG: 2,
    },
    311: {
        name: 'Type 2 Land-based Reconnaissance Aircraft',
        name_jp: '二式陸上偵察機',
        type: "LAND_BASED_SCOUT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_SCOUT,
        AA: 3,
        ACC: 2,
        LOS: 8,
    },
    312: {
        name: 'Type 2 Land-based Reconnaissance Aircraft (Skilled)',
        name_jp: '二式陸上偵察機(熟練)',
        type: "LAND_BASED_SCOUT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_SCOUT,
        AA: 3,
        AR: 1,
        ACC: 3,
        LOS: 9,
    },
    313: {
        name: '5inch Single Gun Mount Mk.30 Kai',
        name_jp: '5inch単装砲 Mk.30改',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 3,
        AA: 7,
        AR: 1,
        ACC: 2,
        EV: 2,
        RNG: 1,
    },
    314: {
        name: '533mm Quintuple Torpedo Mount (Initial Model)',
        name_jp: '533mm五連装魚雷(初期型)',
        type: "TORPEDO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO,
        FP: 1,
        TP: 8,
        AR: 1,
        EV: 1,
        RNG: 1,
    },
    315: {
        name: 'SG Radar (Initial Model)',
        name_jp: 'SG レーダー(初期型)',
        type: "RADAR_S",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_S_MODEL_C,
        FP: 1,
        AA: 3,
        ACC: 8,
        EV: 6,
        ASW: 3,
        LOS: 8,
        RNG: 2,
    },
    316: {
        name: 'Re.2001 CB Kai',
        name_jp: 'Re.2001 CB改',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        FP: 3,
        AERIAL_BOMB: 6,
        AA: 4,
        ACC: 1,
        EV: 2,
        ASW: 3,
    },
    317: {
        name: 'Type 3 Shell Kai',
        name_jp: '三式弾改',
        type: "TYPE_3_SHELL",
        aaci_type: 'A_TYPE3SHELL',
        improvement_type: EquipImprovementType.TYPE_3_SHELL,
        FP: 3,
        AA: 6,
        ACC: 1,
    },
    318: {
        name: '41cm Twin Gun Mount Kai Ni',
        name_jp: '41cm連装砲改二',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 3,
        FP: 21,
        AA: 5,
        AR: 2,
        ACC: 5,
        EV: 1,
        RNG: 3,
    },
    319: {
        name: 'Suisei Model 12 (634 Air Group w/ Type 3 Cluster Bombs)',
        name_jp: '彗星一二型(六三四空/三号爆弾搭載機)',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        can_shell_install_bomber: true,
        AA_resist_ship: 0.6,
        AERIAL_BOMB: 12,
        AA: 3,
        ACC: 1,
        EV: 1,
        ASW: 2,
    },
    320: {
        name: 'Suisei Model 12 (w/ Type 31 Photoelectric Fuze Bombs)',
        name_jp: '彗星一二型(三一号光電管爆弾搭載機)',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        b_type: 'B_NIGHT_BOMBER_DUALROLE',
        AA_resist_ship: 0.6,
        can_shell_install_bomber: true,
        AERIAL_BOMB: 11,
        ACC: 5,
    },
    322: {
        name: 'Zuiun Kai Ni (634 Air Group)',
        name_jp: '瑞雲改二(六三四空)',
        type: "SEAPLANE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_BOMBER,
        AA_resist_ship: .5,
        AA_resist_fleet: .7,
        FP: 2,
        AERIAL_BOMB: 10,
        AA: 4,
        ACC: 2,
        EV: 1,
        ASW: 6,
        LOS: 7,
    },
    323: {
        name: 'Zuiun Kai Ni (634 Air Group/Skilled)',
        name_jp: '瑞雲改二(六三四空/熟練)',
        type: "SEAPLANE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_BOMBER,
        AA_resist_ship: .5,
        AA_resist_fleet: .5,
        FP: 2,
        AERIAL_BOMB: 11,
        AA: 5,
        ACC: 3,
        EV: 3,
        ASW: 7,
        LOS: 8,
    },
    324: {
        name: 'O Type Observation Autogyro Kai',
        name_jp: 'オ号観測機改',
        type: "AUTOGYRO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.AUTOGYRO_LOW,
        FP: 1,
        ACC: 1,
        ASW: 10,
    },
    325: {
        name: 'O Type Observation Autogyro Kai Ni',
        name_jp: 'オ号観測機改二',
        type: "AUTOGYRO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.AUTOGYRO_HIGH,
        FP: 1,
        ACC: 1,
        ASW: 11,
        LOS: 1,
    },
    326: {
        name: 'S-51J',
        name_jp: 'S-51J',
        type: "AUTOGYRO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.AUTOGYRO_HIGH,
        FP: 2,
        ACC: 2,
        ASW: 12,
        LOS: 3,
    },
    327: {
        name: 'S-51J Kai',
        name_jp: 'S-51J改',
        type: "AUTOGYRO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.AUTOGYRO_HIGH,
        FP: 2,
        ACC: 3,
        ASW: 13,
        LOS: 4,
    },
    328: {
        name: '35.6cm Twin Gun Mount Kai',
        name_jp: '35.6cm連装砲改',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 1,
        FP: 16,
        AA: 4,
        AR: 2,
        ACC: 3,
        EV: 2,
        RNG: 3,
    },
    329: {
        name: '35.6cm Twin Gun Mount Kai Ni',
        name_jp: '35.6cm連装砲改二',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        fit_class: 1,
        FP: 17,
        AA: 5,
        AR: 2,
        ACC: 5,
        EV: 2,
        RNG: 3,
    },
    330: {
        name: '16inch Mk.I Twin Gun Mount',
        name_jp: '16inch Mk.I連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        FP: 20,
        AR: 1,
        ACC: 1,
        EV: 1,
        RNG: 3,
    },
    331: {
        name: '16inch Mk.V Twin Gun Mount',
        name_jp: '16inch Mk.V連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        FP: 21,
        AA: 2,
        AR: 2,
        ACC: 2,
        EV: 1,
        RNG: 3,
    },
    332: {
        name: '16inch Mk.VIII Twin Gun Mount Kai',
        name_jp: '16inch Mk.VIII連装砲改',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        FP: 21,
        AA: 6,
        AR: 2,
        ACC: 4,
        EV: 2,
        RNG: 3,
    },
    333: {
        name: 'Reppuu Kai',
        name_jp: '烈風改',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        AA: 10,
        AR: 1,
        AB: 6,
        IN: 2,
    },
    334: {
        name: 'Reppuu Kai (352 Air Group/Skilled)',
        name_jp: '烈風改(三五二空/熟練)',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        AA: 11,
        AR: 1,
        AB: 7,
        IN: 3,
    },
    335: {
        name: 'Reppuu Kai (Prototype Carrier-based Model)',
        name_jp: '烈風改(試製艦載型)',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 1,
        AA: 10,
        AR: 1,
        ACC: 2,
        EV: 3,
    },
    336: {
        name: 'Reppuu Kai Ni',
        name_jp: '烈風改二',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 1,
        AA: 13,
        ACC: 2,
        EV: 1,
    },
    337: {
        name: 'Reppuu Kai Ni (CarDiv 1/Skilled)',
        name_jp: '烈風改二(一航戦/熟練)',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 2,
        AA: 14,
        ACC: 2,
        EV: 1,
    },
    338: {
        name: 'Reppuu Kai Ni Model E',
        name_jp: '烈風改二戊型',
        icon: SpecialIcon.NightFighters,
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        b_type: 'B_NIGHT_FIGHTER',
        FP: 2,
        AA: 11,
        EV: 2,
        LOS: 1,
    },
    339: {
        name: 'Reppuu Kai Ni Model E (CarDiv 1/Skilled)',
        name_jp: '烈風改二戊型(一航戦/熟練)',
        icon: SpecialIcon.NightFighters,
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        b_type: 'B_NIGHT_FIGHTER',
        FP: 2,
        AA: 12,
        EV: 3,
        LOS: 1,
    },
    340: {
        name: '152mm/55 Triple Rapid Fire Gun Mount',
        name_jp: '152mm/55 三連装速射砲',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 8,
        AA: 2,
        AR: 1,
        ACC: 1,
        RNG: 2,
    },
    341: {
        name: '152mm/55 Triple Rapid Fire Gun Mount Kai',
        name_jp: '152mm/55 三連装速射砲改',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 9,
        AA: 3,
        AR: 1,
        ACC: 2,
        EV: 1,
        RNG: 2,
    },
    342: {
        name: 'Ryuusei Kai (CarDiv 1)',
        name_jp: '流星改(一航戦)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AERIAL_TP: 14,
        AA: 2,
        ACC: 1,
        ASW: 6,
        LOS: 4,
    },
    343: {
        name: 'Ryuusei Kai (CarDiv 1/Skilled)',
        name_jp: '流星改(一航戦/熟練)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_TP: 15,
        AA: 3,
        ACC: 2,
        ASW: 7,
        LOS: 6,
    },
    344: {
        name: 'Prototype Type 97 Torpedo Bomber Kai Type 3 Model E (w/ Type 6 Airborne Radar Kai)',
        name_jp: '九七式艦攻改 試製三号戊型(空六号電探改装備機)',
        icon: SpecialIcon.NightTorpedoBombers,
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        b_type: 'B_NIGHT_TORPEDO_BOMBER',
        AERIAL_TP: 7,
        ACC: 1,
        ASW: 6,
        LOS: 4,
    },
    345: {
        name: 'Prototype Type 97 Torpedo Bomber Kai (Skilled) Type 3 Model E (w/ Type 6 Airborne Radar Kai)',
        name_jp: '九七式艦攻改(熟練) 試製三号戊型(空六号電探改装備機)',
        icon: SpecialIcon.NightTorpedoBombers,
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        b_type: 'B_NIGHT_TORPEDO_BOMBER',
        AERIAL_TP: 9,
        ACC: 2,
        ASW: 7,
        LOS: 5,
    },
    346: {
        name: 'Type 2 12cm Mortar Kai',
        name_jp: '二式12cm迫撃砲改',
        type: "DEPTH_CHARGE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DEPTH_CHARGE,
        EV: 1,
        ASW: 3,
    },
    347: {
        name: 'Type 2 12cm Mortar Kai (Concentrated Deployment)',
        name_jp: '二式12cm迫撃砲改 集中配備',
        type: "DEPTH_CHARGE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DEPTH_CHARGE,
        AR: -1,
        EV: 1,
        ASW: 7,
    },
    348: {
        name: 'Shipborne Model Type 4 20cm Anti-ground Rocket Launcher',
        name_jp: '艦載型 四式20cm対地噴進砲',
        type: "WG42",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.WG42,
        AR: -2,
    },
    349: {
        name: 'Type 4 20cm Anti-ground Rocket Launcher (Concentrated Deployment)',
        name_jp: '四式20cm対地噴進砲 集中配備',
        type: "WG42",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.WG42,
        FP: 1,
        AR: -4,
    },
    350: {
        name: 'Me163B',
        name_jp: 'Me163B',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        is_rocket_fighter: true,
        AA: 2,
        AB: 9,
    },
    351: {
        name: 'Prototype Shuusui',
        name_jp: '試製 秋水',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        is_rocket_fighter: true,
        AA: 2,
        AB: 8,
    },
    352: {
        name: 'Shuusui',
        name_jp: '秋水',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        is_rocket_fighter: true,
        AA: 3,
        AB: 9,
    },
    353: {
        name: 'Fw190 A-5 Kai (Skilled)',
        name_jp: 'Fw190 A-5改(熟練)',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 2,
        AA: 11,
        ACC: 1,
        EV: 3,
    },
    354: {
        name: 'Fw190 D-9',
        name_jp: 'Fw190 D-9',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        FP: 2,
        AA: 12,
        AB: 3,
        IN: 3,
    },
    355: {
        name: 'M4A1 DD',
        name_jp: 'M4A1 DD',
        type: "LANDING_CRAFT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LANDING_CRAFT,
        FP: 2,
        EV: -1,
    },
    356: {
        name: '8inch Triple Gun Mount Mk.9',
        name_jp: '8inch三連装砲 Mk.9',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 11,
        AA: 1,
        EV: -1,
        RNG: 2,
    },
    357: {
        name: '8inch Triple Gun Mount Mk.9 mod.2',
        name_jp: '8inch三連装砲 Mk.9 mod.2',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 12,
        AA: 2,
        ACC: 1,
        EV: -1,
        RNG: 2,
    },
    358: {
        name: '5inch Single High-angle Gun Mount Battery',
        name_jp: '5inch 単装高角砲群',
        type: "SECONDARY_GUN",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 2,
        AA: 6,
        AR: 1,
        ACC: 1,
        EV: 1,
        RNG: 1,
    },
    359: {
        name: '6inch Twin Rapid Fire Gun Mount Mk.XXI',
        name_jp: '6inch 連装速射砲 Mk.XXI',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        fit_class: 102,
        FP: 5,
        AA: 3,
        AR: 1,
        ACC: 2,
        EV: 1,
        RNG: 2,
    },
    360: {
        name: 'Bofors 15cm Twin Rapid Fire Gun Mount Mk.9 Model 1938',
        name_jp: 'Bofors 15cm連装速射砲 Mk.9 Model 1938',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        fit_class: 102,
        FP: 6,
        AA: 5,
        ACC: 3,
        EV: 2,
        RNG: 2,
    },
    361: {
        name: 'Bofors 15cm Twin Rapid Fire Gun Mount Mk.9 Kai + Single Rapid Fire Gun Mount Mk.10 Kai Model 1938',
        name_jp: 'Bofors 15cm連装速射砲 Mk.9改+単装速射砲 Mk.10改 Model 1938',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        fit_class: 102,
        FP: 7,
        AA: 6,
        AR: -1,
        ACC: 3,
        EV: 3,
        RNG: 2,
    },
    362: {
        name: '5inch Twin Dual-purpose Gun Mount (Concentrated Deployment)',
        name_jp: '5inch連装両用砲(集中配備)',
        type: "MAIN_GUN_M",
        aaci_type: 'A_HAFD',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 5,
        AA: 11,
        ACC: 2,
        EV: 2,
        RNG: 2,
    },
    363: {
        name: 'GFCS Mk.37 + 5inch Twin Dual-purpose Gun Mount (Concentrated Deployment)',
        name_jp: 'GFCS Mk.37+5inch連装両用砲(集中配備)',
        type: "MAIN_GUN_M",
        aaci_type: 'A_HAFD',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 6,
        AA: 14,
        ACC: 6,
        EV: 3,
        LOS: 3,
        RNG: 2,
    },
    364: {
        name: 'Type D Kouhyouteki Kai (Kouryuu Kai)',
        name_jp: '甲標的 丁型改(蛟龍改)',
        type: "MIDGET_SUBMARINE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.MIDGET_SUBMARINE,
        TP: 17,
        ACC: 2,
        LOS: 2,
    },
    365: {
        name: 'Type 1 Armor Piercing Shell Kai',
        name_jp: '一式徹甲弾改',
        type: "AP_SHELL",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.AP_SHELL,
        FP: 11,
        ACC: 2,
    },
    366: {
        name: '12.7cm Twin Gun Mount Model D Kai 3',
        name_jp: '12.7cm連装砲D型改三',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 3,
        AA: 4,
        AR: 1,
        ACC: 2,
        EV: 2,
        ASW: 2,
        RNG: 1,
    },
    367: {
        name: 'Swordfish (Seaplane Model)',
        name_jp: 'Swordfish(水上機型)',
        type: "SEAPLANE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_BOMBER,
        FP: 2,
        AERIAL_BOMB: 5,
        TP: 5,
        ACC: 1,
        ASW: 4,
        LOS: 2,
    },
    368: {
        name: 'Swordfish Mk.III Kai (Seaplane Model)',
        name_jp: 'Swordfish Mk.III改(水上機型)',
        type: "SEAPLANE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_BOMBER,
        FP: 4,
        AERIAL_BOMB: 7,
        TP: 6,
        ACC: 2,
        ASW: 7,
        LOS: 3,
    },
    369: {
        name: 'Swordfish Mk.III Kai (Seaplane Model/Skilled)',
        name_jp: 'Swordfish Mk.III改(水上機型/熟練)',
        type: "SEAPLANE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_BOMBER,
        FP: 4,
        AERIAL_BOMB: 8,
        TP: 7,
        ACC: 3,
        ASW: 8,
        LOS: 4,
    },
    370: {
        name: 'Swordfish Mk.II Kai (Reconnaissance Seaplane Model)',
        name_jp: 'Swordfish Mk.II改(水偵型)',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        FP: 1,
        ACC: 2,
        ASW: 6,
        LOS: 5,
    },
    371: {
        name: 'Fairey Seafox Kai',
        name_jp: 'Fairey Seafox改',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        FP: 2,
        AA: 1,
        ACC: 2,
        EV: 1,
        ASW: 3,
        LOS: 6,
    },
    372: {
        name: 'Tenzan Model 12A',
        name_jp: '天山一二型甲',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AERIAL_TP: 9,
        AA: 1,
        ASW: 4,
        LOS: 2,
    },
    373: {
        name: 'Tenzan Model 12 A Kai (w/ Type 6 Airborne Radar Kai)',
        name_jp: '天山一二型甲改(空六号電探改装備機)',
        icon: SpecialIcon.NightTorpedoBombers,
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        b_type: 'B_NIGHT_TORPEDO_BOMBER',
        AERIAL_TP: 11,
        AA: 1,
        ACC: 1,
        ASW: 6,
        LOS: 5,
    },
    374: {
        name: 'Tenzan Model 12 A Kai (Skilled/ w/ Type 6 Airborne Radar Kai)',
        name_jp: '天山一二型甲改(熟練/空六号電探改装備機)',
        icon: SpecialIcon.NightTorpedoBombers,
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        b_type: 'B_NIGHT_TORPEDO_BOMBER',
        AA_resist_ship: 0.6,
        AERIAL_TP: 13,
        AA: 1,
        ACC: 2,
        ASW: 7,
        LOS: 6,
    },
    375: {
        name: 'XF5U',
        name_jp: 'XF5U',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 12,
        EV: 2,
        LOS: 1,
    },
    376: {
        name: '533mm Quintuple Torpedo Mount (Late Model)',
        name_jp: '533mm五連装魚雷(後期型)',
        type: "TORPEDO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO,
        FP: 1,
        TP: 11,
        AR: 1,
        EV: 1,
        RNG: 1,
    },
    377: {
        name: 'RUR-4A Weapon Alpha Kai',
        name_jp: 'RUR-4A Weapon Alpha改',
        type: "DEPTH_CHARGE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DEPTH_CHARGE,
        is_DCP: true,
        can_ASW_penetrate: true,
        AR: 1,
        ACC: 1,
        EV: 1,
        ASW: 17,
    },
    378: {
        name: 'Lightweight ASW Torpedo (Initial Test Model)',
        name_jp: '対潜短魚雷(試作初期型)',
        type: "DEPTH_CHARGE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DEPTH_CHARGE,
        is_DC_only: true,
        can_ASW_penetrate: true,
        TP: 2,
        ASW: 20,
    },
    379: {
        name: '12.7cm Single High-angle Gun Mount Kai Ni',
        name_jp: '12.7cm単装高角砲改二',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 1,
        AA: 5,
        ACC: 1,
        EV: 3,
        ASW: 2,
        RNG: 1,
    },
    380: {
        name: '12.7cm Twin High-angle Gun Mount Kai Ni',
        name_jp: '12.7cm連装高角砲改二',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 2,
        AA: 6,
        ACC: 1,
        EV: 2,
        ASW: 2,
        RNG: 1,
    },
    381: {
        name: '16inch Triple Gun Mount Mk.6',
        name_jp: '16inch三連装砲 Mk.6',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        FP: 22,
        AA: 2,
        AR: 1,
        ACC: 2,
        RNG: 3,
    },
    382: {
        name: '12cm Single High-angle Gun Mount Model E',
        name_jp: '12cm単装高角砲E型',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 1,
        AA: 3,
        EV: 2,
        ASW: 1,
        RNG: 1,
    },
    383: {
        name: 'Late Model 53cm Bow Torpedo Mount (8 tubes)',
        name_jp: '後期型53cm艦首魚雷(8門)',
        type: "TORPEDO_SS",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_SS,
        is_special_submarine_CI_torigger: true,
        TP: 19,
        ACC: 3,
        EV: 1,
        RNG: 1,
    },
    384: {
        name: 'Late Model Submarine Radar & Passive Radiolocator',
        name_jp: '後期型潜水艦搭載電探&逆探',
        type: "SUBMARINE_RADAR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SUBMARINE_RADAR,
        FP: 2,
        TP: 4,
        AR: 1,
        ACC: 4,
        EV: 13,
        LOS: 7,
    },
    385: {
        name: '16inch Triple Gun Mount Mk.6 mod.2',
        name_jp: '16inch三連装砲 Mk.6 mod.2',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        FP: 23,
        AA: 2,
        AR: 2,
        ACC: 3,
        RNG: 3,
    },
    386: {
        name: '6inch Triple Rapid Fire Gun Mount Mk.16',
        name_jp: '6inch三連装速射砲 Mk.16',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 7,
        AA: 2,
        AR: 1,
        ACC: 2,
        RNG: 2,
    },
    387: {
        name: '6inch Triple Rapid Fire Gun Mount Mk.16 mod.2',
        name_jp: '6inch三連装速射砲 Mk.16 mod.2',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 8,
        AA: 3,
        AR: 1,
        ACC: 4,
        EV: 1,
        RNG: 2,
    },
    388: {
        name: 'Ginga (Egusa Squadron)',
        name_jp: '銀河(江草隊)',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_BOMB: 15,
        AERIAL_TP: 15,
        AA: 3,
        ACC: 3,
        ASW: 4,
        LOS: 4,
    },
    389: {
        name: 'TBM-3W+3S',
        name_jp: 'TBM-3W+3S',
        icon: SpecialIcon.NightTorpedoBombers,
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        b_type: 'B_NIGHT_TORPEDO_BOMBER',
        FP: 3,
        AERIAL_BOMB: 7,
        AERIAL_TP: 10,
        ACC: 3,
        ASW: 13,
        LOS: 10,
    },
    390: {
        name: '16inch Triple Gun Mount Mk.6 + GFCS',
        name_jp: '16inch三連装砲 Mk.6+GFCS',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        FP: 23,
        AA: 3,
        AR: 2,
        ACC: 6,
        EV: 1,
        RNG: 3,
    },
    391: {
        name: 'Type 99 Dive Bomber Model 22',
        name_jp: '九九式艦爆二二型',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        can_shell_install_bomber: true,
        AERIAL_BOMB: 6,
        ACC: 1,
        ASW: 3,
        LOS: 2,
    },
    392: {
        name: 'Type 99 Dive Bomber Model 22 (Skilled)',
        name_jp: '九九式艦爆二二型(熟練)',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        can_shell_install_bomber: true,
        AERIAL_BOMB: 8,
        AA: 1,
        ACC: 3,
        ASW: 4,
        LOS: 3,
    },
    393: {
        name: '120mm/50 Twin Gun Mount mod.1936',
        name_jp: '120mm/50 連装砲 mod.1936',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 3,
        AA: 3,
        ACC: 2,
        RNG: 1,
    },
    394: {
        name: '120mm/50 Twin Gun Mount Kai A.mod.1937',
        name_jp: '120mm/50 連装砲改 A.mod.1937',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 3,
        AA: 4,
        ACC: 3,
        RNG: 1,
    },
    395: {
        name: 'Shinzan',
        name_jp: '深山',
        type: "LAND_BASED_BOMBER_L",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER_L,
        AERIAL_BOMB: 17,
        AERIAL_TP: 16,
        AA: 1,
    },
    396: {
        name: 'Shinzan Kai',
        name_jp: '深山改',
        type: "LAND_BASED_BOMBER_L",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER_L,
        AERIAL_BOMB: 19,
        AERIAL_TP: 17,
        AA: 2,
    },
    397: {
        name: 'Locally Modified 12.7cm Twin High-angle Gun Mount',
        name_jp: '現地改装12.7cm連装高角砲',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 3,
        AA: 6,
        AR: 1,
        ACC: 2,
        RNG: 1,
    },
    398: {
        name: 'Locally Modified 10cm Twin High-angle Gun Mount',
        name_jp: '現地改装10cm連装高角砲',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAFD',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 2,
        AA: 8,
        ACC: 1,
        EV: 2,
        RNG: 1,
    },
    399: {
        name: '6inch Mk.XXIII Triple Gun Mount',
        name_jp: '6inch Mk.XXIII三連装砲',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 7,
        AA: 2,
        AR: 1,
        ACC: 1,
        EV: 1,
        RNG: 2,
    },
    400: {
        name: '533mm Triple Torpedo Mount (Model 53-39)',
        name_jp: '533mm 三連装魚雷(53-39型)',
        type: "TORPEDO",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO,
        FP: 1,
        TP: 6,
        ACC: 1,
        EV: 2,
        RNG: 1,
    },
    401: {
        name: 'Do 17 Z-2',
        name_jp: 'Do 17 Z-2',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AERIAL_BOMB: 11,
        AA: 2,
        ASW: 5,
        LOS: 2,
    },
    402: {
        name: 'Arctic Gear & Deck Personnel',
        name_jp: '寒冷地装備&甲板要員',
        type: "SCAMP",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SCAMP,
    },
    403: {
        name: 'Type 4 Heavy Bomber Hiryuu',
        name_jp: '四式重爆 飛龍',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AERIAL_BOMB: 15,
        AERIAL_TP: 14,
        AA: 5,
        ACC: 1,
        ASW: 4,
        LOS: 4,
    },
    404: {
        name: 'Type 4 Heavy Bomber Hiryuu (Skilled)',
        name_jp: '四式重爆 飛龍(熟練)',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_BOMB: 16,
        AERIAL_TP: 14,
        AA: 5,
        ACC: 2,
        ASW: 5,
        LOS: 5,
    },
    405: {
        name: 'Do 217 E-5 + Hs293 Initial Model',
        name_jp: 'Do 217 E-5+Hs293初期型',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_BOMB: 22,
        AERIAL_TP: 13,
        AA: 4,
        ACC: 2,
        LOS: 3,
    },
    406: {
        name: 'Do 217 K-2 + Fritz-X',
        name_jp: 'Do 217 K-2+Fritz-X',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_BOMB: 24,
        AERIAL_TP: 16,
        AA: 4,
        ACC: 2,
        LOS: 3,
    },
    407: {
        name: '15.2cm Twin Gun Mount Kai Ni',
        name_jp: '15.2cm連装砲改二',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        fit_class: 102,
        FP: 6,
        AA: 4,
        AR: 1,
        ACC: 4,
        RNG: 2,
    },
    408: {
        name: 'Soukoutei (Armored Boat Class)',
        name_jp: '装甲艇(AB艇)',
        type: "LANDING_CRAFT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LANDING_CRAFT,
        FP: 1,
        AR: 1,
        LOS: 1,
    },
    409: {
        name: 'Armed Daihatsu',
        name_jp: '武装大発',
        type: "LANDING_CRAFT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LANDING_CRAFT,
        FP: 1,
        AA: 1,
        ACC: 1,
    },
    410: {
        name: 'Type 21 Air Radar Kai Ni',
        name_jp: '21号対空電探改二',
        type: "RADAR_L",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_L,
        AA: 7,
        ACC: 4,
        EV: 4,
        LOS: 7,
    },
    411: {
        name: 'Type 42 Air Radar Kai Ni',
        name_jp: '42号対空電探改二',
        type: "RADAR_L",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_L,
        AA: 7,
        ACC: 8,
        EV: -1,
        LOS: 6,
    },
    412: {
        name: 'Torpedo Squadron Skilled Lookouts',
        name_jp: '水雷戦隊 熟練見張員',
        type: "PICKET",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.PICKET,
        TP: 3,
        AA: 1,
        ACC: 2,
        EV: 3,
        LOS: 2,
        RNG: 2,
    },
    413: {
        name: 'Elite Torpedo Squadron Command Facility',
        name_jp: '精鋭水雷戦隊 司令部',
        type: "FCF",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FCF,
        TP: 3,
        ACC: 2,
        LOS: 1,
    },
    414: {
        name: 'SOC Seagull',
        name_jp: 'SOC Seagull',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        AERIAL_BOMB: 1,
        ACC: 2,
        EV: 1,
        ASW: 1,
        LOS: 4,
    },
    415: {
        name: 'SO3C Seamew Kai',
        name_jp: 'SO3C Seamew改',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        AA: 1,
        ACC: 3,
        ASW: 2,
        LOS: 7,
    },
    416: {
        name: 'Type 0 Fighter Model 21 (Tainan Air Group)',
        name_jp: '零式艦戦21型(台南空)',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        AA: 11,
        AB: 1,
        IN: 3,
        LOS: 1,
    },
    417: {
        name: 'Type 0 Fighter Model 32 (Tainan Air Group)',
        name_jp: '零式艦戦32型(台南空)',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        AA: 12,
        AB: 1,
        IN: 4,
        LOS: 1,
    },
    418: {
        name: 'Type 0 Fighter Model 22 (251 Air Group)',
        name_jp: '零式艦戦22型(251空)',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        AA: 12,
        AB: 1,
        IN: 3,
        LOS: 1,
    },
    419: {
        name: 'SBD-5',
        name_jp: 'SBD-5',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        FP: 1,
        AERIAL_BOMB: 7,
        AA: 2,
        ACC: 2,
        EV: 3,
        ASW: 4,
        LOS: 3,
    },
    420: {
        name: 'SB2C-3',
        name_jp: 'SB2C-3',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        can_shell_install_bomber: true,
        FP: 1,
        AERIAL_BOMB: 11,
        AA: 2,
        ACC: 1,
        EV: 1,
        ASW: 5,
        LOS: 3,
    },
    421: {
        name: 'SB2C-5',
        name_jp: 'SB2C-5',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        can_shell_install_bomber: true,
        FP: 2,
        AERIAL_BOMB: 12,
        AA: 2,
        ACC: 2,
        EV: 2,
        ASW: 6,
        LOS: 4,
    },
    422: {
        name: 'FR-1 Fireball',
        name_jp: 'FR-1 Fireball',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 2,
        AA: 11,
        ACC: 1,
        EV: 3,
    },
    423: {
        name: 'Fulmar (Reconnaissance Fighter/Skilled)',
        name_jp: 'Fulmar(戦闘偵察/熟練)',
        type: "CARRIER_SCOUT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.CARRIER_SCOUT,
        FP: 2,
        AA: 4,
        ACC: 3,
        EV: 3,
        ASW: 3,
        LOS: 4,
    },
    424: {
        name: 'Barracuda Mk.II',
        name_jp: 'Barracuda Mk.II',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AERIAL_TP: 7,
        ACC: 1,
        ASW: 5,
        LOS: 2,
    },
    425: {
        name: 'Barracuda Mk.III',
        name_jp: 'Barracuda Mk.III',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AERIAL_TP: 6,
        ACC: 1,
        ASW: 9,
        LOS: 3,
    },
    426: {
        name: '305mm/46 Twin Gun Mount',
        name_jp: '305mm/46 連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        FP: 13,
        AR: 1,
        ACC: 1,
        EV: 2,
        RNG: 3,
    },
    427: {
        name: '305mm/46 Triple Gun Mount',
        name_jp: '305mm/46 三連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        FP: 14,
        AR: 1,
        ACC: 2,
        EV: 1,
        RNG: 3,
    },
    428: {
        name: '320mm/44 Twin Gun Mount',
        name_jp: '320mm/44 連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        FP: 15,
        AR: 1,
        ACC: 1,
        EV: 2,
        RNG: 3,
    },
    429: {
        name: '320mm/44 Triple Gun Mount',
        name_jp: '320mm/44 三連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        FP: 16,
        AR: 1,
        ACC: 2,
        EV: 1,
        RNG: 3,
    },
    430: {
        name: '65mm/64 Single Rapid Fire Gun Mount Kai',
        name_jp: '65mm/64 単装速射砲改',
        type: "SECONDARY_GUN",
        aaci_type: 'A_HAFD',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 1,
        AA: 9,
        EV: 1,
        RNG: 1,
    },
    431: {
        name: 'SM.79',
        name_jp: 'SM.79',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AERIAL_BOMB: 13,
        AERIAL_TP: 9,
        AA: 2,
        ASW: 3,
        LOS: 2,
    },
    432: {
        name: 'SM.79 bis',
        name_jp: 'SM.79 bis',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AERIAL_BOMB: 14,
        AERIAL_TP: 12,
        AA: 3,
        ASW: 4,
        LOS: 3,
    },
    433: {
        name: 'SM.79 bis (Skilled)',
        name_jp: 'SM.79 bis(熟練)',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_BOMB: 14,
        AERIAL_TP: 13,
        AA: 3,
        ACC: 2,
        ASW: 4,
        LOS: 3,
    },
    434: {
        name: 'Corsair Mk.II',
        name_jp: 'Corsair Mk.II',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 1,
        AA: 10,
        EV: 1,
        LOS: 1,
    },
    435: {
        name: 'Corsair Mk.II (Ace)',
        name_jp: 'Corsair Mk.II(Ace)',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 2,
        AA: 11,
        ACC: 2,
        EV: 2,
        LOS: 2,
    },
    436: {
        name: 'Daihatsu Landing Craft (Panzer II / North African Specification)',
        name_jp: '大発動艇(II号戦車/北アフリカ仕様)',
        type: "LANDING_CRAFT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LANDING_CRAFT,
        FP: 1,
        ACC: 1,
    },
    437: {
        name: 'Prototype Jinpuu',
        name_jp: '試製 陣風',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        AA: 13,
        ACC: 1,
        EV: 1,
    },
    438: {
        name: 'Type 3 Active Sonar Kai',
        name_jp: '三式水中探信儀改',
        type: "SONAR_S",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SONAR_S,
        AR: 1,
        ACC: 2,
        EV: 1,
        ASW: 11,
    },
    439: {
        name: 'Hedgehog (Initial Model)',
        name_jp: 'Hedgehog(初期型)',
        type: "DEPTH_CHARGE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DEPTH_CHARGE,
        is_DC_only: true,
        can_ASW_penetrate: true,
        AR: -1,
        ACC: 3,
        ASW: 18,
    },
    440: {
        name: '21inch 6-tube Bow Torpedo Launcher (Initial Model)',
        name_jp: '21inch艦首魚雷発射管6門(初期型)',
        type: "TORPEDO_SS",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_SS,
        TP: 10,
        EV: 2,
        RNG: 1,
    },
    441: {
        name: '21inch 6-tube Bow Torpedo Launcher (Late Model)',
        name_jp: '21inch艦首魚雷発射管6門(後期型)',
        type: "TORPEDO_SS",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_SS,
        is_special_submarine_CI_torigger: true,
        TP: 14,
        ACC: 3,
        EV: 3,
        RNG: 1,
    },
    442: {
        name: 'Submarine 4-tube Stern Torpedo Launcher (Initial Model)',
        name_jp: '潜水艦後部魚雷発射管4門(初期型)',
        type: "TORPEDO_SS",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_SS,
        TP: 6,
        ACC: 1,
        EV: 1,
        RNG: 1,
    },
    443: {
        name: 'Submarine 4-tube Stern Torpedo Launcher (Late Model)',
        name_jp: '潜水艦後部魚雷発射管4門(後期型)',
        type: "TORPEDO_SS",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_SS,
        is_special_submarine_CI_torigger: true,
        TP: 9,
        ACC: 2,
        EV: 2,
        RNG: 1,
    },
    444: {
        name: 'Type 4 Heavy Bomber Hiryuu + No.1 Model 1A Guided Missile',
        name_jp: '四式重爆 飛龍+イ号一型甲 誘導弾',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_BOMB: 20,
        AERIAL_TP: 15,
        AA: 5,
        ACC: 1,
        ASW: 3,
        LOS: 4,
    },
    445: {
        name: 'Type 2 Two-seat Fighter Toryuu',
        name_jp: '二式複戦 屠龍',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        icon: SpecialIcon.ArmyLandBasedFighters,
        AA: 3,
        AR: 1,
        AB: 4,
        IN: 1,
    },
    446: {
        name: 'Type 2 Two-seat Fighter Toryuu Model C',
        name_jp: '二式複戦 屠龍 丙型',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        icon: SpecialIcon.ArmyLandBasedFighters,
        AA: 3,
        AR: 2,
        AB: 6,
        IN: 2,
    },
    447: {
        name: 'Type 0 Fighter Model 64 (Two-seat w/ KMX)',
        name_jp: '零式艦戦64型(複座KMX搭載機)',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        AERIAL_BOMB: 3,
        AA: 4,
        ASW: 8,
        LOS: 3,
    },
    449: {
        name: 'Toku Daihatsu Landing Craft + Type 1 Gun Tank',
        name_jp: '特大発動艇+一式砲戦車',
        type: "LANDING_CRAFT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LANDING_CRAFT,
        b_type: 'B_LC2',
        FP: 2,
    },
    450: {
        name: 'Type 13 Air Radar Kai (Late Model)',
        name_jp: '13号対空電探改(後期型)',
        type: "RADAR_S",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_S_MODEL_A,
        AA: 6,
        ACC: 2,
        EV: 3,
        LOS: 5,
    },
    451: {
        name: 'Type 3 Command Liaison Aircraft Kai',
        name_jp: '三式指揮連絡機改',
        type: "ASW_PLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.ASW_PLANE_HIGH,
        FP: 1,
        ACC: 2,
        ASW: 9,
        LOS: 2,
    },
    452: {
        name: 'Ki-96',
        name_jp: 'キ96',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        icon: SpecialIcon.ArmyLandBasedFighters,
        AA: 5,
        AR: 2,
        AB: 5,
        IN: 4,
    },
    453: {
        name: 'Ki-102 B',
        name_jp: 'キ102乙',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        icon: SpecialIcon.LandBasedRaidAircraft,
        AA_resist_ship: 0.6,
        FP: 4,
        AERIAL_BOMB: 19,
        AERIAL_TP: 11,
        AA: 4,
        ACC: 4,
        ASW: 4,
    },
    454: {
        name: 'Ki-102 B Kai + No.1 Model 1B Guided Missile',
        name_jp: 'キ102乙改+イ号一型乙 誘導弾',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        icon: SpecialIcon.LandBasedRaidAircraft,
        AA_resist_ship: 0.6,
        AA_resist_fleet: .7,
        FP: 4,
        AERIAL_BOMB: 20,
        AERIAL_TP: 14,
        AA: 3,
        ACC: 3,
        ASW: 4,
    },
    455: {
        name: 'Prototype Long-barrel 12.7cm Twin Gun Mount Model A Kai 4',
        name_jp: '試製 長12.7cm連装砲A型改四',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 4,
        AA: 3,
        AR: 1,
        ACC: 2,
        RNG: 1,
    },
    456: {
        name: 'SG Radar (Late Model)',
        name_jp: 'SG レーダー(後期型)',
        type: "RADAR_S",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_S_MODEL_C,
        FP: 2,
        AA: 3,
        ACC: 10,
        EV: 6,
        ASW: 4,
        LOS: 9,
        RNG: 2,
    },
    457: {
        name: 'Late Model Bow Torpedo Mount (4 tubes)',
        name_jp: '後期型艦首魚雷(4門)',
        type: "TORPEDO_SS",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_SS,
        is_special_submarine_CI_torigger: true,
        TP: 12,
        AR: 1,
        ACC: 3,
        EV: 3,
        RNG: 1,
    },
    458: {
        name: 'Late Model Radar & Passive Radiolocator + Snorkel Equipment',
        name_jp: '後期型電探&逆探+シュノーケル装備',
        type: "SUBMARINE_RADAR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SUBMARINE_RADAR,
        TP: 4,
        AR: 2,
        ACC: 4,
        EV: 15,
        LOS: 6,
    },
    459: {
        name: 'B-25',
        name_jp: 'B-25',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        is_skip_bomber: true,
        FP: 3,
        AERIAL_BOMB: 16,
        AERIAL_TP: 8,
        AA: 4,
        LOS: 4,
    },
    460: {
        name: '15m Duplex Rangefinder Kai + Type 21 Radar Kai Ni + Skilled Fire Direction Center',
        name_jp: '15m二重測距儀改+21号電探改二+熟練射撃指揮所',
        type: "RADAR_XL",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_XL,
        FP: 4,
        AA: 8,
        AR: 1,
        ACC: 10,
        EV: 2,
        LOS: 8,
    },
    461: {
        name: 'Skilled Sonar Personnel + Late Model Bow Torpedo Mount (4 tubes)',
        name_jp: '熟練聴音員+後期型艦首魚雷(4門)',
        type: "TORPEDO_SS",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_SS,
        is_special_submarine_CI_torigger: true,
        TP: 13,
        AR: 1,
        ACC: 5,
        EV: 5,
        RNG: 1,
    },
    463: {
        name: '15.5cm Triple Secondary Gun Mount Kai Ni',
        name_jp: '15.5cm三連装副砲改二',
        type: "SECONDARY_GUN",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        FP: 9,
        AA: 5,
        AR: 1,
        ACC: 5,
        EV: 1,
        RNG: 2,
    },
    464: {
        name: '10cm Twin High-angle Gun Mount Battery Concentrated Deployment',
        name_jp: '10cm連装高角砲群 集中配備',
        type: "SECONDARY_GUN",
        aaci_type: 'A_HAFD',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 3,
        AA: 12,
        AR: 2,
        ACC: 2,
        EV: 2,
        RNG: 1,
    },
    465: {
        name: 'Prototype 51cm Triple Gun Mount',
        name_jp: '試製51cm三連装砲',
        type: "MAIN_GUN_XL",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_XL,
        FP: 36,
        AA: 5,
        AR: 2,
        ACC: -2,
        EV: -10,
        RNG: 4,
    },
    466: {
        name: 'Ryuusei Kai (Skilled)',
        name_jp: '流星改(熟練)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_TP: 13,
        AA: 3,
        ACC: 1,
        ASW: 6,
        LOS: 5,
    },
    467: {
        name: '5inch Twin Gun Mount (Secondary Armament) Concentrated Deployment',
        name_jp: '5inch連装砲(副砲配置) 集中配備',
        type: "SECONDARY_GUN_L",
        aaci_type: 'A_HAFD',
        improvement_type: EquipImprovementType.SECONDARY_GUN_L,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 5,
        AA: 11,
        AR: 1,
        ACC: 4,
        EV: 3,
        RNG: 1,
    },
    468: {
        name: '38cm Quadruple Gun Mount Kai Deux',
        name_jp: '38cm四連装砲改 deux',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        FP: 24,
        AA: 5,
        AR: 1,
        ACC: 4,
        RNG: 2,
    },
    469: {
        name: 'Type 0 Reconnaissance Seaplane Model 11B Kai (Night Recon)',
        name_jp: '零式水上偵察機11型乙改(夜偵)',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        icon: SpecialIcon.NightScout,
        is_night_scout: true,
        FP: 1,
        AA: 1,
        ACC: 2,
        ASW: 6,
        LOS: 5,
    },
    470: {
        name: '12.7cm Twin Gun Mount Model C Kai 3',
        name_jp: '12.7cm連装砲C型改三',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 3,
        AA: 2,
        AR: 1,
        ACC: 1,
        EV: 1,
        ASW: 3,
        RNG: 1,
    },
    471: {
        name: 'Loire 130M',
        name_jp: 'Loire 130M',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        icon: SpecialIcon.NightScout,
        is_night_scout: true,
        ACC: 2,
        ASW: 3,
        LOS: 3,
    },
    472: {
        name: 'Mk.32 ASW Torpedo (Mk.2 Thrower)',
        name_jp: 'Mk.32 対潜魚雷(Mk.2落射機)',
        type: "DEPTH_CHARGE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DEPTH_CHARGE,
        is_DCP: true,
        can_ASW_penetrate: true,
        TP: 1,
        ACC: 1,
        ASW: 19,
    },
    473: {
        name: 'F4U-2 Night Corsair',
        name_jp: 'F4U-2 Night Corsair',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        icon: SpecialIcon.NightFighters,
        b_type: 'B_NIGHT_FIGHTER',
        FP: 1,
        AA: 9,
        ACC: 1,
        EV: 2,
        ASW: 5,
        LOS: 2,
    },
    474: {
        name: 'F4U-4',
        name_jp: 'F4U-4',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        AA_resist_ship: 0.6,
        can_shell_install_bomber: true,
        FP: 2,
        AERIAL_BOMB: 6,
        AA: 10,
        EV: 1,
        ASW: 3,
        LOS: 2,
    },
    475: {
        name: 'AU-1',
        name_jp: 'AU-1',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        AA_resist_ship: 0.6,
        FP: 3,
        AERIAL_BOMB: 12,
        AERIAL_TP: 6,
        AA: 9,
        EV: 1,
        ASW: 5,
        LOS: 2,
    },
    476: {
        name: 'F4U-7',
        name_jp: 'F4U-7',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        AA_resist_ship: 0.6,
        FP: 3,
        AERIAL_BOMB: 11,
        AERIAL_TP: 8,
        AA: 10,
        EV: 2,
        ASW: 4,
        LOS: 2,
    },
    477: {
        name: 'Skilled Deck Personnel',
        name_jp: '熟練甲板要員',
        type: "SCAMP",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SCAMP,
        FP: 2,
        AA: 1,
        AR: 1,
        ACC: 1,
        EV: 1,
        LOS: 1,
    },
    478: {
        name: 'Skilled Deck Personnel + Aviation Maintenance Hands',
        name_jp: '熟練甲板要員+航空整備員',
        type: "SCAMP",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SCAMP,
        FP: 7,
        AERIAL_BOMB: 1,
        TP: 1,
        AA: 1,
        AR: 1,
        ACC: 1,
        EV: 1,
        ASW: 1,
        LOS: 1,
        RNG: 4,
    },
    479: {
        name: 'Mosquito FB Mk.VI',
        name_jp: 'Mosquito FB Mk.VI',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AA_resist_ship: 0.6,
        AA_resist_fleet: .7,
        AERIAL_BOMB: 18,
        AERIAL_TP: 5,
        AA: 5,
        ACC: 1,
        ASW: 4,
        LOS: 5,
    },
    480: {
        name: 'Mosquito PR Mk.IV',
        name_jp: 'Mosquito PR Mk.IV',
        type: "LAND_BASED_SCOUT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_SCOUT,
        AR: 2,
        ACC: 2,
        LOS: 9,
    },
    481: {
        name: 'Mosquito TR Mk.33',
        name_jp: 'Mosquito TR Mk.33',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_TP: 8,
        AA: 5,
        ACC: 1,
        ASW: 6,
        LOS: 6,
    },
    482: {
        name: 'Toku Daihatsu Landing Craft + Panzer III (North African Specification)',
        name_jp: '特大発動艇+Ⅲ号戦車(北アフリカ仕様)',
        type: "LANDING_CRAFT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LANDING_CRAFT,
        FP: 1,
        AA: 1,
    },
    483: {
        name: 'Type 3 Shell Kai Ni',
        name_jp: '三式弾改二',
        type: "TYPE_3_SHELL",
        aaci_type: 'A_TYPE3SHELL',
        improvement_type: EquipImprovementType.TYPE_3_SHELL,
        FP: 5,
        AA: 7,
        ACC: 1,
        EV: 1,
    },
    484: {
        name: 'Type 4 Heavy Bomber Hiryuu (Skilled) + No.1 Model 1A Guided Missile',
        name_jp: '四式重爆 飛龍(熟練)+イ号一型甲 誘導弾',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_BOMB: 21,
        AERIAL_TP: 17,
        AA: 5,
        ACC: 3,
        ASW: 4,
        LOS: 5,
    },
    485: {
        name: 'Kyoufuu Kai Ni',
        name_jp: '強風改二',
        type: "SEAPLANE_FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_FIGHTER,
        AA: 6,
        ACC: 1,
        EV: 4,
        LOS: 2,
    },
    486: {
        name: 'Type 0 Fighter Model 64 (Air Superiority Fighter Specification)',
        name_jp: '零式艦戦64型(制空戦闘機仕様)',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 1,
        AA: 9,
        ACC: 1,
        EV: 3,
        LOS: 1,
    },
    487: {
        name: 'Type 0 Fighter Model 64 (Skilled Fighter-Bomber)',
        name_jp: '零式艦戦64型(熟練爆戦)',
        type: "FIGHTER_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER_BOMBER,
        AA_resist_ship: 0.6,
        FP: 1,
        AERIAL_BOMB: 6,
        AA: 7,
        ACC: 2,
        ASW: 3,
        LOS: 2,
    },
    488: {
        name: 'Type 2 Depth Charge Kai Ni',
        name_jp: '二式爆雷改二',
        type: "DEPTH_CHARGE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DEPTH_CHARGE,
        is_DC_only: true,
        can_ASW_penetrate: true,
        AR: 1,
        ACC: 1,
        EV: 1,
        ASW: 8,
    },
    489: {
        name: 'Type 1 Fighter Hayabusa Model II Kai (20th Squadron)',
        name_jp: '一式戦 隼II型改(20戦隊)',
        type: "ASW_PLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.ASW_PLANE_HIGH,
        is_20th_family: true,
        FP: 2,
        AERIAL_BOMB: 4,
        AA: 6,
        EV: 3,
        ASW: 8,
    },
    490: {
        name: 'Prototype Night Zuiun (Attack Equipment)',
        name_jp: '試製 夜間瑞雲(攻撃装備)',
        type: "SEAPLANE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE_BOMBER,
        icon: SpecialIcon.NightSeaplaneBombers,
        AA_resist_ship: .5,
        AA_resist_fleet: .7,
        FP: 3,
        AERIAL_BOMB: 10,
        AA: 4,
        ACC: 2,
        ASW: 6,
        LOS: 7,
    },
    491: {
        name: 'Type 1 Fighter Hayabusa Model III Kai (Skilled / 20th Squadron)',
        name_jp: '一式戦 隼III型改(熟練/20戦隊)',
        type: "ASW_PLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.ASW_PLANE_HIGH,
        is_20th_family: true,
        AA_resist_ship: 0.6,
        FP: 2,
        AERIAL_BOMB: 5,
        AA: 7,
        ACC: 1,
        EV: 3,
        ASW: 8,
    },
    492: {
        name: 'Zero Fighter Model 52C (Hachiman Force)',
        name_jp: '零戦52型丙(八幡部隊)',
        type: "FIGHTER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 1,
        AA: 10,
        ACC: 1,
        EV: 1,
    },
    493: {
        name: 'Type 1 Land-based Attack Aircraft (Hachiman Force)',
        name_jp: '一式陸攻(八幡部隊)',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AERIAL_BOMB: 12,
        AERIAL_TP: 11,
        AA: 3,
        ACC: 1,
        ASW: 2,
        LOS: 4,
    },
    494: {
        name: 'Toku Daihatsu Landing Craft + Chi-Ha',
        name_jp: '特大発動艇+チハ',
        type: "LANDING_CRAFT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LANDING_CRAFT,
    },
    495: {
        name: 'Toku Daihatsu Landing Craft + Chi-Ha Kai',
        name_jp: '特大発動艇+チハ改',
        type: "LANDING_CRAFT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LANDING_CRAFT,
        FP: 1,
    },
    496: {
        name: 'Army Infantry Corps',
        name_jp: '陸軍歩兵部隊',
        type: "ARMY_UNIT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.ARMY_UNIT,
    },
    497: {
        name: 'Type 97 Medium Tank (Chi-Ha)',
        name_jp: '九七式中戦車(チハ)',
        type: "ARMY_UNIT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.ARMY_UNIT,
    },
    498: {
        name: 'Type 97 Medium Tank New Turret (Chi-Ha Kai)',
        name_jp: '九七式中戦車 新砲塔(チハ改)',
        type: "ARMY_UNIT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.ARMY_UNIT,
    },
    499: {
        name: 'Army Infantry Corps + Chi-Ha Kai',
        name_jp: '陸軍歩兵部隊+チハ改',
        type: "ARMY_UNIT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.ARMY_UNIT,
    },
    500: {
        name: 'Smoke Generator (Smoke Screen)',
        name_jp: '発煙装置(煙幕)',
        type: "SMOKESCREEN",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SMOKESCREEN,
        EV: 1,
    },
    501: {
        name: 'Smoke Generator Kai (Smoke Screen)',
        name_jp: '発煙装置改(煙幕)',
        type: "SMOKESCREEN",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SMOKESCREEN,
        EV: 3,
    },
    502: {
        name: '35.6cm Twin Gun Mount Kai 3 (Dazzle Camouflage)',
        name_jp: '35.6cm連装砲改三(ダズル迷彩仕様)',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        FP: 20,
        AA: 6,
        AR: 2,
        ACC: 4,
        EV: 3,
        RNG: 3,
    },
    503: {
        name: '35.6cm Twin Gun Mount Kai 4',
        name_jp: '35.6cm連装砲改四',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        FP: 22,
        AA: 5,
        AR: 2,
        ACC: 4,
        EV: 1,
        RNG: 3,
    },
    504: {
        name: 'Ginga (Skilled)',
        name_jp: '銀河(熟練)',
        type: "LAND_BASED_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LAND_BASED_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_BOMB: 15,
        AERIAL_TP: 14,
        AA: 3,
        ACC: 2,
        ASW: 3,
        LOS: 3,
    },
    505: {
        name: '25mm Anti-Aircraft Autocannon Expansion',
        name_jp: '25mm対空機銃増備',
        type: "AA_GUN",
        aaci_type: 'A_AAGUN',
        improvement_type: EquipImprovementType.AA_GUN,
        AA: 7,
        EV: 2,
    },
    506: {
        name: 'Radar Equipment Mast (Type 13 Kai + Type 22 Radar Kai 4)',
        name_jp: '電探装備マスト(13号改+22号電探改四)',
        type: "RADAR_S",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_S_MODEL_A,
        AA: 4,
        ACC: 6,
        EV: 2,
        ASW: 2,
        LOS: 5,
    },
    507: {
        name: '14inch/45 Twin Gun Mount',
        name_jp: '14inch/45 連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        FP: 16,
        AR: 1,
        EV: 1,
        RNG: 3,
    },
    508: {
        name: '14inch/45 Triple Gun Mount',
        name_jp: '14inch/45 三連装砲',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        FP: 19,
        AR: 1,
        RNG: 3,
    },
    509: {
        name: '12cm Single High-angle Gun Mount Model E Kai',
        name_jp: '12cm単装高角砲E型改',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 1,
        AA: 4,
        AR: 1,
        ACC: 1,
        EV: 3,
        ASW: 2,
        RNG: 1,
    },
    510: {
        name: 'Walrus',
        name_jp: 'Walrus',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        icon: SpecialIcon.NightScout,
        is_night_scout: true,
        ACC: 1,
        ASW: 4,
        LOS: 4,
    },
    511: {
        name: '21inch 4-tube Bow Torpedo Launcher (Initial Model)',
        name_jp: '21inch艦首魚雷発射管4門(初期型)',
        type: "TORPEDO_SS",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_SS,
        TP: 8,
        EV: 4,
        RNG: 1,
    },
    512: {
        name: '21inch 4-tube Bow Torpedo Launcher (Late Model)',
        name_jp: '21inch艦首魚雷発射管4門(後期型)',
        type: "TORPEDO_SS",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_SS,
        is_special_submarine_CI_torigger: true,
        TP: 12,
        ACC: 3,
        EV: 5,
        RNG: 1,
    },
    513: {
        name: 'Barrage Balloon',
        name_jp: '阻塞気球',
        type: "BARRAGE_BALLOON",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.BARRAGE_BALLOON,
        icon: SpecialIcon.BarrageBalloons,
        EV: 1,
    },
    514: {
        name: 'Toku Daihatsu Landing Craft + Panzer III Ausf. J',
        name_jp: '特大発動艇+Ⅲ号戦車J型',
        type: "LANDING_CRAFT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LANDING_CRAFT,
        b_type: 'B_LC2',
        FP: 2,
        AA: 1,
        ACC: 1,
    },
    515: {
        name: 'Sea Otter',
        name_jp: 'Sea Otter',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        icon: SpecialIcon.NightScout,
        is_night_scout: true,
        ACC: 2,
        ASW: 6,
        LOS: 5,
    },
    516: {
        name: 'Me 262 A-1a/R1',
        name_jp: 'Me 262 A-1a/R1',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        icon: SpecialIcon.JetLandBasedFighters,
        AERIAL_BOMB: 3,
        AA: 15,
        AB: 6,
        IN: 1,
    },
    517: {
        name: 'Passive Radiolocator (E27) + Type 22 Surface Radar Kai 4 (Calibrated Late Model)',
        name_jp: '逆探(E27)+22号対水上電探改四(後期調整型)',
        type: "RADAR_S",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.RADAR_S_MODEL_D,
        FP: 2,
        ACC: 9,
        EV: 5,
        ASW: 4,
        LOS: 8,
    },
    518: {
        name: '14cm Twin Gun Mount Kai 2',
        name_jp: '14cm連装砲改二',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        fit_class: 102,
        FP: 5,
        AR: 2,
        ACC: 4,
        ASW: 3,
        RNG: 2,
    },
    519: {
        name: 'SJ Radar + Submarine Conning Tower Equipment',
        name_jp: 'SJレーダー+潜水艦司令塔装備',
        type: "SUBMARINE_RADAR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SUBMARINE_RADAR,
        TP: 5,
        ACC: 5,
        EV: 6,
        LOS: 7,
    },
    520: {
        name: 'Prototype 20.3cm (No.4) Twin Gun Mount',
        name_jp: '試製20.3cm(4号)連装砲',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 11,
        AA: 4,
        AR: 1,
        ACC: 2,
        RNG: 2,
    },
    521: {
        name: 'Shiun (Skilled)',
        name_jp: '紫雲(熟練)',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        AERIAL_BOMB: 1,
        ACC: 2,
        EV: 2,
        ASW: 4,
        LOS: 9,
    },
    522: {
        name: 'Type 0 Small Reconnaissance Seaplane',
        name_jp: '零式小型水上機',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        AERIAL_BOMB: 1,
        EV: 1,
        ASW: 1,
        LOS: 3,
    },
    523: {
        name: 'Type 0 Small Reconnaissance Seaplane (Skilled)',
        name_jp: '零式小型水上機(熟練)',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        AERIAL_BOMB: 1,
        ACC: 1,
        EV: 2,
        ASW: 2,
        LOS: 4,
    },
    524: {
        name: '12cm Single High-angle Gun + 25mm Machine Gun',
        name_jp: '12cm単装高角砲+25mm機銃増備',
        type: "SECONDARY_GUN",
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 1,
        AA: 5,
        EV: 2,
        ASW: 1,
        RNG: 1,
    },
    525: {
        name: 'Special Type 4 Amphibious Tank',
        name_jp: '特四式内火艇',
        type: "LANDING_TANK",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LANDING_TANK,
        b_type: 'B_OTHER',
        TP: 1,
        AA: 1,
    },
    526: {
        name: 'Special Type 4 Amphibious Tank Kai',
        name_jp: '特四式内火艇改',
        type: "LANDING_TANK",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.LANDING_TANK,
        b_type: 'B_OTHER',
        TP: 3,
        AA: 2,
        ACC: 1,
    },
    527: {
        name: 'Type 281 Radar',
        name_jp: 'Type281 レーダー',
        type: "RADAR_L",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_L,
        AA: 7,
        ACC: 1,
        EV: 2,
        LOS: 8,
    },
    528: {
        name: 'Type 274 Fire Control Radar',
        name_jp: 'Type274 射撃管制レーダー',
        type: "RADAR_L",
        aaci_type: 'A_AIRRADAR',
        improvement_type: EquipImprovementType.RADAR_L,
        FP: 2,
        AA: 3,
        AR: 1,
        ACC: 8,
        EV: 2,
        LOS: 5,
        RNG: 2,
    },
    529: {
        name: '12.7cm Twin Gun Mount Model C Kai 3 H',
        name_jp: '12.7cm連装砲C型改三H',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        FP: 3,
        AA: 5,
        AR: 1,
        ACC: 1,
        EV: 1,
        ASW: 3,
        RNG: 1,
    },
    530: {
        name: '35.6cm Twin Gun Mount Kai 3C',
        name_jp: '35.6cm連装砲改三丙',
        type: "MAIN_GUN_L",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_L,
        FP: 21,
        AA: 3,
        AR: 4,
        ACC: 3,
        RNG: 3,
    },
    531: {
        name: 'Fleet Communication Antenna',
        name_jp: '艦隊通信アンテナ',
        type: "FCF",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FCF,
        ACC: 1,
        LOS: 1,
    },
    532: {
        name: 'Communication Equipment & Personnel',
        name_jp: '通信装置&要員',
        type: "FCF",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FCF,
        LOS: 1,
    },
    533: {
        name: '10cm Twin High-angle Gun Mount Kai + Anti-Aircraft Fire Director Kai',
        name_jp: '10cm連装高角砲改+高射装置改',
        type: "MAIN_GUN_S",
        aaci_type: 'A_HAFD',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 3,
        AA: 11,
        AR: 4,
        ACC: 2,
        EV: 2,
        RNG: 1,
    },
    534: {
        name: '13.8cm Twin Gun Mount',
        name_jp: '13.8cm連装砲',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        fit_class: 102,
        FP: 4,
        AA: 2,
        RNG: 1,
    },
    535: {
        name: '13.8cm Twin Gun Mount Kai',
        name_jp: '13.8cm連装砲改',
        type: "MAIN_GUN_S",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        fit_class: 102,
        FP: 5,
        AA: 2,
        AR: 1,
        ACC: 1,
        RNG: 1,
    },
    536: {
        name: '15.2cm Triple Main Gun Mount',
        name_jp: '15.2cm三連装主砲',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 7,
        AA: 2,
        EV: 1,
        RNG: 2,
    },
    537: {
        name: '15.2cm Triple Main Gun Mount Kai',
        name_jp: '15.2cm三連装主砲改',
        type: "MAIN_GUN_M",
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 8,
        AA: 3,
        ACC: 1,
        EV: 3,
        RNG: 2,
    },
    538: {
        name: 'Loire 130M Kai (Skilled)',
        name_jp: 'Loire 130M改(熟練)',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        icon: SpecialIcon.NightScout,
        is_night_scout: true,
        FP: 2,
        ACC: 3,
        ASW: 5,
        LOS: 5,
    },
    539: {
        name: 'SOC Seagull Late Model (Skilled)',
        name_jp: 'SOC Seagull 後期型(熟練)',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        AERIAL_BOMB: 2,
        ACC: 3,
        EV: 2,
        ASW: 3,
        LOS: 7,
    },
    540: {
        name: 'Type 0 Reconnaissance Seaplane Model 11A Kai 2',
        name_jp: '零式水上偵察機11型甲改二',
        type: "SEAPLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.SEAPLANE,
        icon: SpecialIcon.NightScout,
        is_night_scout: true,
        FP: 1,
        AA: 1,
        ACC: 3,
        ASW: 4,
        LOS: 7,
    },
    541: {
        name: 'SBD (Yellow Wings)',
        name_jp: 'SBD(Yellow Wings)',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        FP: 1,
        AERIAL_BOMB: 7,
        AA: 2,
        ACC: 1,
        EV: 2,
        ASW: 3,
        LOS: 2,
    },
    542: {
        name: 'TBD (Yellow Wings)',
        name_jp: 'TBD(Yellow Wings)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        FP: 1,
        AERIAL_TP: 6,
        ASW: 4,
        LOS: 2,
    },
    543: {
        name: 'SBD VS-2 (Reconnaissance Squadron)',
        name_jp: 'SBD VS-2(偵察飛行隊)',
        type: "CARRIER_SCOUT",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.CARRIER_SCOUT,
        FP: 2,
        AERIAL_BOMB: 6,
        AA: 2,
        ACC: 3,
        EV: 3,
        ASW: 5,
        LOS: 6,
    },
    544: {
        name: 'SBD VB-2 (Bombing Squadron)',
        name_jp: 'SBD VB-2(爆撃飛行隊)',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        FP: 3,
        AERIAL_BOMB: 8,
        AA: 2,
        ACC: 2,
        EV: 3,
        ASW: 4,
        LOS: 2,
    },
    545: {
        name: 'Tenzan Model 12A Kai 2 (Murata Squadron w/ Radar)',
        name_jp: '天山一二型甲改二(村田隊/電探装備)',
        type: "TORPEDO_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        icon: SpecialIcon.NightTorpedoBombers,
        b_type: 'B_NIGHT_TORPEDO_BOMBER',
        AA_resist_ship: 0.6,
        AERIAL_TP: 16,
        AA: 1,
        ACC: 3,
        ASW: 7,
        LOS: 6,
    },
    546: {
        name: 'Prototype Shinden (LB Interceptor)',
        name_jp: '試製 震電(局地戦闘機)',
        type: "INTERCEPTOR",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.INTERCEPTOR,
        icon: SpecialIcon.PropulsionLandBasedFighters,
        AA: 14,
        AB: 5,
        IN: 4,
    },
    549: {
        name: 'Type 3 Command Liaison Aircraft Kai 2',
        name_jp: '三式指揮連絡機改二',
        type: "ASW_PLANE",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.ASW_PLANE_HIGH,
        FP: 2,
        AERIAL_BOMB: 2,
        ACC: 2,
        ASW: 10,
        LOS: 3,
    },
    550: {
        name: 'Prototype Myoujou (Additional Trial Model)',
        name_jp: '試製 明星(増加試作機)',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        AERIAL_BOMB: 4,
        ASW: 4,
        LOS: 1,
    },
    551: {
        name: 'Myoujou Kai',
        name_jp: '明星改',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        AERIAL_BOMB: 5,
        ASW: 5,
        LOS: 2,
    },
    552: {
        name: 'Type 99 Training Bomber Model 22 Kai (Experimental w/ Night Equipment)',
        name_jp: '九九式練爆二二型改(夜間装備実験機)',
        type: "DIVE_BOMBER",
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        icon: SpecialIcon.NightDiveBombers,
        AERIAL_BOMB: 6,
        ACC: 1,
        ASW: 6,
        LOS: 4,
    },
    553: {
        name: '10cm Twin High-angle Gun Mount Kai',
        name_jp: '10cm連装高角砲改',
        type: 'MAIN_GUN_S',
        aaci_type: 'A_HAGUN',
        improvement_type: EquipImprovementType.MAIN_GUN_S,
        icon: SpecialIcon.MainHighAngleGun,
        FP: 2,
        AA: 7,
        AR: 1,
        ACC: 1,
        RNG: 1,
    },
    554: {
        name: 'Type 97 Torpedo Bomber Kai (Northeastern Air Group)',
        name_jp: '九七式艦攻改(北東海軍航空隊)',
        type: 'TORPEDO_BOMBER',
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        TP: 6,
        ASW: 5,
        LOS: 2,
    },
    555: {
        name: '18cm/57 Triple Main Gun Mount',
        name_jp: '18cm/57 三連装主砲',
        type: 'MAIN_GUN_M',
        aaci_type: 'A_GUN',
        improvement_type: EquipImprovementType.MAIN_GUN_M,
        FP: 9,
        AA: 2,
        AR: 1,
        RNG: 2,
    },
    556: {
        name: '10cm/56 Single High-angle Gun Mount (Concentrated Deployment)',
        name_jp: '10cm/56 単装高角砲(集中配備)',
        type: 'SECONDARY_GUN', icon: SpecialIcon.MainHighAngleGun,
        aaci_type: 'A_HAFD',
        improvement_type: EquipImprovementType.SECONDARY_GUN,
        FP: 3,
        AA: 9,
        ACC: 1,
        LOS: 1,
        RNG: 1,
    },
    560: {
        name: 'Bf109 T-3(G)',
        name_jp: 'Bf109 T-3(G)',
        type: 'FIGHTER',
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.FIGHTER,
        FP: 1,
        AA: 9,
        ACC: 2,
        EV: 4,
    },
    557: {
        name: 'Type 0 Fighter Model 62 Kai (Night Fighter-bomber)',
        name_jp: '零式艦戦62型改(夜間爆戦)',
        type: 'DIVE_BOMBER', icon: SpecialIcon.NightDiveBombers,
        aaci_type: 'NONE',
        b_type: 'B_NIGHT_BOMBER',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        AA_resist_ship: 0.6,
        AERIAL_BOMB: 5,
        AA: 4,
        ACC: 1,
        ASW: 3,
        LOS: 1,
    },
    558: {
        name: 'Type 0 Fighter Model 62 Kai (Skilled/Night Fighter-bomber)',
        name_jp: '零式艦戦62型改(熟練/夜間爆戦)',
        type: 'DIVE_BOMBER', icon: SpecialIcon.NightDiveBombers,
        aaci_type: 'NONE',
        b_type: 'B_NIGHT_BOMBER',
        improvement_type: EquipImprovementType.DIVE_BOMBER,
        AA_resist_ship: 0.6,
        AA_resist_fleet: 0.7,
        AERIAL_BOMB: 6,
        AA: 5,
        ACC: 1,
        EV: 2,
        ASW: 4,
        LOS: 2,
    },
    559: {
        name: 'Ju87 D-4(Fliegerass)',
        name_jp: 'Ju87 D-4(Fliegerass)',
        type: 'TORPEDO_BOMBER',
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.TORPEDO_BOMBER,
        AA_resist_ship: .6,
        FP: 4,
        TP: 7,
        AA: 1,
        ACC: 3,
        ASW: 7,
        LOS: 3,
    },
    561: {
        name: 'Ho 229',
        name_jp: 'Ho229',
        type: 'JET_BOMBER', icon: SpecialIcon.FullWingJetBomber,
        aaci_type: 'NONE',
        improvement_type: EquipImprovementType.JET_BOMBER,
        AA_resist_ship: 0.5, //temp
        AA_resist_fleet: 0.5, //temp
        FP: 3,
        AERIAL_BOMB: 16,
        AA: 8,
        LOS: 3,
        RNG: 5,
    },
};